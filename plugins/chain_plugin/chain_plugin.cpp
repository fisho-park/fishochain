...
void chain_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
...
#ifndef FOC_VM_OC_DEVELOPER
            //throwing an exception here (like FOC_ASSERT) is just gobbled up with a "Failed to initialize" error :(
            if(vm == wasm_interface::vm_type::eos_vm_oc) {
               elog("FOC VM OC is a tier-up compiler and works in conjunction with the configured base WASM runtime. Enable FOC VM OC via 'eos-vm-oc-enable' option");
               FOC_ASSERT(false, plugin_exception, "");
            }
#endif
         ...
         ("eos-vm-oc-cache-size-mb", bpo::value<uint64_t>()->default_value(eosvmoc::config().cache_size / (1024u*1024u)), "Maximum size (in MiB) of the FOC VM OC code cache")
         ...
         }), "Number of threads to use for FOC VM OC tier-up")
         ("eos-vm-oc-enable", bpo::bool_switch(), "Enable FOC VM OC tier-up runtime")
#endif
         ;
...
protocol_feature_set initialize_protocol_features( const fc::path& p, bool populate_missing_builtins = true ) {
   ...
   if( fc::exists( p ) ) {
      FOC_ASSERT( fc::is_directory( p ), plugin_exception,
   ...
   if( directory_exists ) {
      for( directory_iterator enditr, itr{p}; itr != enditr; ++itr ) {
         ...
         FOC_ASSERT( res.second, plugin_exception,
   ...
   auto output_protocol_feature = [&p]( const builtin_protocol_feature& f, const digest_type& feature_digest ) {
      ...
      FOC_ASSERT( !fc::exists( file_path ), plugin_exception,
   ...
   ( builtin_protocol_feature_t codename ) -> digest_type {
      ...
      if( !res.second ) {
         FOC_ASSERT( res.first->second, protocol_feature_exception,
   ...
   try {
      ...
      if( options.at( "print-build-info" ).as<bool>() || options.count( "extract-build-info") ) {
         ...
         if( options.count( "extract-build-info") ) {
            ...
            FOC_ASSERT( fc::json::save_to_file( chainbase::environment(), p, true ), misc_exception,
         ...
         FOC_THROW( node_management_success, "reported build environment information" );
      }
      ...
      if( options.count( "action-blacklist" )) {
         ...
         for( const auto& a : acts ) {
            auto pos = a.find( "::" );
            FOC_ASSERT( pos != std::string::npos, plugin_config_exception, "Invalid entry in action-blacklist: '${a}'", ("a", a));
      ...
      if( options.count("checkpoint") ) {
         ...
         for( const auto& cp : cps ) {
            ...
            if( itr != my->loaded_checkpoints.end() ) {
               FOC_ASSERT( itr->second == item.second,
      ...
      if( options.count( "chain-threads" )) {
         ...
         FOC_ASSERT( my->chain_config->thread_pool_size > 0, plugin_config_exception,
      ...
      FOC_ASSERT( my->chain_config->sig_cpu_bill_pct >= 0 && my->chain_config->sig_cpu_bill_pct <= 100, plugin_config_exception,
      ...
      if( options.count( "extract-genesis-json" ) || options.at( "print-genesis-json" ).as<bool>()) {
         ...
         if( options.count( "extract-genesis-json" )) {
            ...
            FOC_ASSERT( fc::json::save_to_file( *gs, p, true ),
                        misc_exception,
         ...
         FOC_THROW( extract_genesis_state_exception, "extracted genesis state from blocks.log" );
      }
      ...
      if( options.count("export-reversible-blocks") ) {
         ...
         FOC_THROW( node_management_success, "exported reversible blocks" );
      }
      ...
      if( options.at( "delete-all-blocks" ).as<bool>()) {
         ...
         FOC_THROW( fixed_reversible_db_exception, "fixed corrupted reversible blocks database" );
         ...
         FOC_THROW( node_management_success, "imported reversible blocks" );
      }
      ...
      if (options.count( "snapshot" )) {
         ...
         FOC_ASSERT( fc::exists(*my->snapshot_path), plugin_config_exception,
         ...
         FOC_ASSERT( !fc::is_regular_file(shared_mem_path),
                 plugin_config_exception,
         ...
         if( fc::is_regular_file( my->blocks_dir / "blocks.log" )) {
            ...
            if( block_log_genesis ) {
               ...
               FOC_ASSERT( *chain_id == block_log_chain_id,
                           plugin_config_exception,
         ...
         if( fc::is_regular_file( my->blocks_dir / "blocks.log" ) ) {
            ...
            if( chain_id ) {
               FOC_ASSERT( *block_log_chain_id == *chain_id, block_log_exception,
         ...
         if( options.count( "genesis-json" ) ) {
            ...
            FOC_ASSERT( fc::is_regular_file( genesis_file ),
                        plugin_config_exception,
            ...
            if( block_log_genesis ) {
               FOC_ASSERT( *block_log_genesis == provided_genesis, plugin_config_exception,
               ...
               if( chain_id ) {
                  FOC_ASSERT( provided_genesis_chain_id == *chain_id, plugin_config_exception,
                  ...
                  if( block_log_chain_id ) {
                     FOC_ASSERT( provided_genesis_chain_id == *block_log_chain_id, plugin_config_exception,
                                 "Genesis state, provided via command line arguments, has a chain ID (${provided_genesis_chain_id}) "
         ...
         if( !chain_id ) {
            if( my->genesis ) {
               ...
               FOC_ASSERT( !block_log_chain_id, plugin_config_exception,
      ...
      my->pre_accepted_block_connection = my->chain->pre_accepted_block.connect([this](const signed_block_ptr& blk) {
         ...
         if( itr != my->loaded_checkpoints.end() ) {
            auto id = blk->id();
            FOC_ASSERT( itr->second == id, checkpoint_exception,
...
void chain_apis::read_write::validate() const {
   FOC_ASSERT( db.get_read_mode() != chain::db_read_mode::READ_ONLY, missing_chain_api_plugin_exception, "Not allowed, node in read-only mode" );
}
...
bool chain_plugin::recover_reversible_blocks( const fc::path& db_dir, uint32_t cache_size,
   ...
   if( new_db_dir ) {
      ...
      FOC_ASSERT( reversible_dir_name != ".", invalid_reversible_blocks_dir, "Invalid path to reversible directory" );
      backup_dir = reversible_dir.parent_path() / reversible_dir_name.append("-").append( now );

      FOC_ASSERT( !fc::exists(backup_dir),
   ...
   try {
      for( ; itr != ubi.end(); ++itr ) {
         FOC_ASSERT( itr->blocknum == end + 1, gap_in_reversible_blocks_db,
   ...
   try {
      while( reversible_blocks.tellg() < end_pos ) {
         ...
         if( start == 0 ) {
            ...
            FOC_ASSERT( num == end + 1, gap_in_reversible_blocks_db,
...
void chain_plugin::handle_guard_exception(const chain::guard_exception& e) {
   ...
   elog("database chain::guard_exception, quiting..."); // log string searched for in: tests/nodefoc_under_min_avail_ram.py
   // quit the app
   app().quit();
}
...
void chain_plugin::handle_db_exhaustion() {
   ...
   //return 1 -- it's what programs/nodefoc/main.cpp considers "BAD_ALLOC"
   std::_Exit(1);
}

void chain_plugin::handle_bad_alloc() {
   elog("std::bad_alloc - memory exhausted");
   //return -2 -- it's what programs/nodefoc/main.cpp reports for std::exception
   std::_Exit(-2);
}
...
uint64_t read_only::get_table_index_name(const read_only::get_table_rows_params& p, bool& primary) {
   ...
   FOC_ASSERT( index == table, chain::contract_table_query_exception, "Unsupported table name: ${n}", ("n", p.table) );
   ...
   if (p.index_position.empty() || p.index_position == "first" || p.index_position == "primary" || p.index_position == "one") {
      ...
      try {
         ...
         FOC_ASSERT( false, chain::contract_table_query_exception, "Invalid index_position: ${p}", ("p", p.index_position));
      }
      if (pos < 2) {
         primary = true;
         pos = 0;
...
uint64_t convert_to_type(const string& str, const string& desc) {
   ...
   if (str.find(',') != string::npos) { // fix #6274 only match formats like 4,FOC
      try {
   ...
   try {
      ...
      FOC_ASSERT( false, chain_type_exception, "Could not convert ${desc} string '${str}' to any of the following: "
...
double convert_to_type(const string& str, const string& desc) {
   ...
   FOC_ASSERT( !std::isnan(val), chain::contract_table_query_exception,
...
string convert_to_string(const chain::key256_t& source, const string& key_type, const string& encode_type, const string& desc) {
   try {
      ...
      FOC_ASSERT( false, chain_type_exception, "Incompatible key_type and encode_type for key256_t next_key" );
...
abi_def get_abi( const controller& db, const name& account ) {
   ...
   FOC_ASSERT(code_accnt != nullptr, chain::account_query_exception, "Fail to retrieve account for ${account}", ("account", account) );
   abi_def abi;
...
string get_table_type( const abi_def& abi, const name& table_name ) {
   ...
   FOC_ASSERT( false, chain::contract_table_query_exception, "Table ${table} is not specified in the ABI", ("table",table_name) );
}
...
#pragma GCC diagnostic ignored "-Wstrict-aliasing"
   ...
   if( primary ) {
      FOC_ASSERT( p.table == table_with_index, chain::contract_table_query_exception, "Invalid table name ${t}", ( "t", p.table ));
      auto table_type = get_table_type( abi, p.table );
      if( table_type == KEYi64 || p.key_type == "i64" || p.key_type == "name" ) {
         return get_table_rows_ex<key_value_index>(p,abi);
      }
      FOC_ASSERT( false, chain::contract_table_query_exception,  "Invalid table type ${type}", ("type",table_type)("abi",abi));
   } else {
      FOC_ASSERT( !p.key_type.empty(), chain::contract_table_query_exception, "key type required for non-primary index" );
      ...
      FOC_ASSERT(false, chain::contract_table_query_exception,  "Unsupported secondary index type: ${t}", ("t", p.key_type));
   }
...
vector<asset> read_only::get_currency_balance( const read_only::get_currency_balance_params& p )const {
   ...
   walk_key_value_table(p.code, p.account, N(accounts), [&](const key_value_object& obj){
      FOC_ASSERT( obj.value.size() >= sizeof(asset), chain::asset_type_exception, "Invalid data on table");

      asset cursor;
      fc::datastream<const char *> ds(obj.value.data(), obj.value.size());
      fc::raw::unpack(ds, cursor);

      FOC_ASSERT( cursor.get_symbol().valid(), chain::asset_type_exception, "Invalid asset");
...
fc::variant read_only::get_currency_stats( const read_only::get_currency_stats_params& p )const {
   ...
   walk_key_value_table(p.code, name(scope), N(stat), [&](const key_value_object& obj){
      FOC_ASSERT( obj.value.size() >= sizeof(read_only::get_currency_stats_result), chain::asset_type_exception, "Invalid data on table");
...
fc::variant get_global_row( const database& db, const abi_def& abi, const abi_serializer& abis, const fc::microseconds& abi_serializer_max_time_ms, bool shorten_abi_errors ) {
   ...
   FOC_ASSERT(table_type == read_only::KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table global", ("type",table_type));
   ...
   FOC_ASSERT(table_id, chain::contract_table_query_exception, "Missing table global");
   ...
   FOC_ASSERT(it != kv_index.end(), chain::contract_table_query_exception, "Missing row in table global");
...
read_only::get_producers_result read_only::get_producers( const read_only::get_producers_params& p ) const try {
   ...
   FOC_ASSERT(table_type == KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table producers", ("type",table_type));
   ...
   FOC_ASSERT(table_id && secondary_table_id, chain::contract_table_query_exception, "Missing producers table");
      ...
      if (!p.lower_bound.empty()) {
         try {
            ...
            try {
               ...
               if (itr == by_txid.end()) {
                  FOC_THROW(transaction_exception, "Unknown Transaction ID: ${txid}", ("txid", txid));
               }
...
fc::variant read_only::get_block(const read_only::get_block_params& params) const {
   ...
   FOC_ASSERT( !params.block_num_or_id.empty() && params.block_num_or_id.size() <= 64,
   ...
   if( block_num.valid() ) {
      ...
      } FOC_RETHROW_EXCEPTIONS(chain::block_id_type_exception, "Invalid block ID: ${block_num_or_id}", ("block_num_or_id", params.block_num_or_id))
   }

   FOC_ASSERT( block, unknown_block_exception, "Could not find block: ${block}", ("block", params.block_num_or_id));
...
fc::variant read_only::get_block_header_state(const get_block_header_state_params& params) const {
   ...
   FOC_ASSERT( b, unknown_block_exception, "Could not find reversible block: ${block}", ("block", params.block_num_or_id));
...
void read_write::push_transaction(const read_write::push_transaction_params& params, next_function<read_write::push_transaction_results> next) {
   try {
      ...
      } FOC_RETHROW_EXCEPTIONS(chain::packed_transaction_type_exception, "Invalid packed transaction")
...
void read_write::push_transactions(const read_write::push_transactions_params& params, next_function<read_write::push_transactions_results> next) {
   try {
      FOC_ASSERT( params.size() <= 1000, too_many_tx_at_once, "Attempt to push too many transactions at once" );
...
read_only::get_code_results read_only::get_code( const get_code_params& params )const {
   ...
   FOC_ASSERT( params.code_as_wasm, unsupported_feature, "Returning WAST from get_code is no longer supported" );
...
read_only::get_account_results read_only::get_account( const get_account_params& params )const {
   ...
   while( perm != permissions.end() && perm->owner == params.account_name ) {
      ...
      if( perm->parent._id ) {
         ...
         if( p ) {
            FOC_ASSERT(perm->owner == p->owner, invalid_parent_permission, "Invalid parent permission");
            parent = p->name;
         }
      }
   ...
   if( abi_serializer::to_abi(code_account.abi, abi) ) {
      ...
      const auto token_code = N(foc.token);
...
read_only::abi_json_to_bin_result read_only::abi_json_to_bin( const read_only::abi_json_to_bin_params& params )const try {
   ...
   FOC_ASSERT(code_account != nullptr, contract_query_exception, "Contract can't be found ${contract}", ("contract", params.code));

   abi_def abi;
   if( abi_serializer::to_abi(code_account->abi, abi) ) {
      abi_serializer abis( abi, abi_serializer_max_time );
      auto action_type = abis.get_action_type(params.action);
      FOC_ASSERT(!action_type.empty(), action_validate_exception, "Unknown action ${action} in contract ${contract}", ("action", params.action)("contract", params.code));
      try {
         result.binargs = abis.variant_to_binary( action_type, params.args, abi_serializer_max_time, shorten_abi_errors );
      } FOC_RETHROW_EXCEPTIONS(chain::invalid_action_args_exception,
                                "'${args}' is invalid args for action '${action}' code '${code}'. expected '${proto}'",
                                ("args", params.args)("action", params.action)("code", params.code)("proto", action_abi_to_variant(abi, action_type)))
   } else {
      FOC_ASSERT(false, abi_not_found_exception, "No ABI found for ${contract}", ("contract", params.code));
   }
   return result;
...
read_only::get_required_keys_result read_only::get_required_keys( const get_required_keys_params& params )const {
   ...
   } FOC_RETHROW_EXCEPTIONS(chain::transaction_type_exception, "Invalid transaction")
