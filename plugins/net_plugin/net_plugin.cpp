  ...
  FC_MULTILINE_MACRO_END
   ...
   public:
      ...
      void fill_out_buffer( std::vector<boost::asio::const_buffer>& bufs ) {
         ...
         if( _sync_write_queue.size() > 0 ) { // always send msgs from sync_write_queue first
            ...
            FOC_ASSERT( _write_queue_size == 0, plugin_exception, "write queue size expected to be zero" );
         }
      }
   ...
   struct msg_handler : public fc::visitor<void> {
      ...
      void operator()( const T& ) const {
         FOC_ASSERT( false, plugin_config_exception, "Not implemented, call handle_message directly instead" );
      }
   ...
   void sync_manager::sync_recv_notice( const connection_ptr& c, const notice_message& msg) {
      ...
      FOC_ASSERT( msg.known_blocks.mode == catch_up || msg.known_blocks.mode == last_irr_catch_up, plugin_exception,
              ...
              [conn = shared_from_this(), socket=socket]( boost::system::error_code ec, std::size_t bytes_transferred ) {
               ...
               try {
                  if( !ec ) {
                     ...
                     FOC_ASSERT(bytes_transferred <= conn->pending_message_buffer.bytes_to_write(), plugin_exception, "");
   ...
   void net_plugin::set_program_options( options_description& /*cli*/, options_description& cfg )
   {
      cfg.add_options()
         ...
         ( "agent-name", bpo::value<string>()->default_value("\"FOC Test Agent\""), "The name supplied to identify this node amongst the peers.")
   ...
   void net_plugin::plugin_initialize( const variables_map& options ) {
      ...
      try {
         ...
         if( options.count( "p2p-listen-endpoint" ) && options.at("p2p-listen-endpoint").as<string>().length()) {
            ...
            FOC_ASSERT( my->p2p_address.length() <= max_p2p_address_length, chain::plugin_config_exception,
         ...
         if( options.count( "p2p-server-address" ) ) {
            ...
            FOC_ASSERT( my->p2p_server_address.length() <= max_p2p_address_length, chain::plugin_config_exception,
         ...
         FOC_ASSERT( my->thread_pool_size > 0, chain::plugin_config_exception,
         ...
         if( options.count( "agent-name" )) {
            ...
            FOC_ASSERT( my->user_agent_name.length() <= max_handshake_str_length, chain::plugin_config_exception,
         ...
         if( my->allowed_connections & net_plugin_impl::Specified )
            FOC_ASSERT( options.count( "peer-key" ),
         ...
         FOC_ASSERT( my->chain_plug, chain::missing_chain_plugin_exception, ""  );
