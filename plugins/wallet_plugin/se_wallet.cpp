...
#define STR(A) #A
   ...
   bool remove_key(string public_key) {
      ...
      macos_user_auth(auth_callback, &prom, CFSTR("remove a key from your FOC wallet"));
      if(!fut.get())
...
static void check_signed() {
   ...
   if(is_valid != errSecSuccess) {
      ...
      FOC_THROW(secure_enclave_exception, "");
   }
}
...
se_wallet::se_wallet() : my(new detail::se_wallet_impl()) {
   ...
   FOC_THROW(secure_enclave_exception, "Secure Enclave not supported on this hardware");
}
...
void se_wallet::lock() {
   FOC_ASSERT(!is_locked(), wallet_locked_exception, "You can not lock an already locked wallet");
   my->locked = true;
}
...
void se_wallet::unlock(string password) {
   promise<bool> prom;
   future<bool> fut = prom.get_future();
   macos_user_auth(detail::auth_callback, &prom, CFSTR("unlock your FOC wallet"));
   if(!fut.get())
      FC_THROW_EXCEPTION(chain::wallet_invalid_password_exception, "Local user authentication failed");
   my->locked = false;
}
...
string se_wallet::create_key(string key_type) {
   FOC_ASSERT(key_type.empty() || key_type == "R1", chain::unsupported_key_type_exception, "Secure Enclave wallet only supports R1 keys");
   return (string)my->create();
}

bool se_wallet::remove_key(string key) {
   FOC_ASSERT(!is_locked(), wallet_locked_exception, "You can not remove a key from a locked wallet");
