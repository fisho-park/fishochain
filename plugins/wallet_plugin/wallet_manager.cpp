   ...
   t->async_wait([t, this](const boost::system::error_code& /*ec*/)
   {
      ...
      if(ec != boost::system::error_code()) {
         if(rc.type() == bfs::file_not_found) {
            appbase::app().quit();
            FOC_THROW(wallet_exception, "Lock file removed while keosd still running.  Terminating.");
         }
      }
...
void wallet_manager::initialize_lock() {
   ...
   if(!wallet_dir_lock->try_lock()) {
      ...
      FOC_THROW(wallet_exception, "Failed to lock access to wallet directory; is another keosd running?");
   }
