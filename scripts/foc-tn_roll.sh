#!/bin/bash
#
# foc-tn_roll is used to have all of the instances of the FOC daemon on a host brought down
# so that the underlying executable image file (the "text file") can be replaced. Then
# all instances are restarted.
# usage: foc-tn_roll.sh [arglist]
...
# the command line.
#
# In most cases, simply running ./foc-tn_roll.sh is sufficient.
#

if [ -z "$FOC_HOME" ]; then
    echo FOC_HOME not set - $0 unable to proceed.
    exit -1
fi
...
prog=""
RD=""
for p in eosd eosiod nodefoc; do
    prog=$p
    RD=bin
    if [ -f $RD/$prog ]; then
        break;
...
if [ \( -z "$prog" \) -o \( -z "$RD" \) ]; then
    echo unable to locate binary for eosd or eosiod or nodefoc
    exit 1
fi
...
if [ $DD = "all" ]; then
    for FOC_RESTART_DATA_DIR in `ls -d var/lib/node_??`; do
        bash $FOC_HOME/scripts/foc-tn_up.sh "$*"
    done
else
    bash $FOC_HOME/scripts/foc-tn_up.sh "$*"
fi
unset FOC_RESTART_DATA_DIR
