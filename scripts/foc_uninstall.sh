...
export FOC_INSTALL_DIR=${INSTALL_LOCATION:-$FOC_INSTALL_DIR}
   ...
   while true; do
      ...
      case $PROCEED in
         ...
         0 | true | [Yy]* )
            ...
            echo " - FOC Removal Complete"
            break;;
         1 | false | [Nn]* ) echo " - Cancelled FOC Removal!"; exit 1;;
         * ) echo "Please type 'y' for yes or 'n' for no.";;
      esac
   done
fi

echo "[Removing FOC Dependencies]"
if [[ $ARCH == "Darwin" ]]; then
   for package in $(cat scripts/foc_build_darwin_deps | cut -d, -f1 2>/dev/null); do
      while true; do
