...
if [[ -z "${NAME}" ]]; then
    ...
    else echo " ${COLOR_RED}- FOC is not supported for your Architecture!${COLOR_NC}" && exit 1
    fi
fi

# Setup yum and apt variables
if [[ $NAME =~ "Amazon Linux" ]] || [[ $NAME == "CentOS Linux" ]]; then
    if ! YUM=$( command -v yum 2>/dev/null ); then echo "${COLOR_RED}YUM must be installed to compile FOC${COLOR_NC}" && exit 1; fi
elif [[ $NAME == "Ubuntu" ]]; then
    if ! APTGET=$( command -v apt-get 2>/dev/null ); then echo "${COLOR_RED}APT-GET must be installed to compile FOC${COLOR_NC}" && exit 1; fi
fi
...
function setup() {
    if $VERBOSE; then
        ...
        echo "FOC_INSTALL_DIR: ${FOC_INSTALL_DIR}"
...
function ensure-which() {
  if ! which ls &>/dev/null; then
    while true; do
      [[ $NONINTERACTIVE == false ]] && printf "${COLOR_YELLOW}FOC compiler checks require the 'which' package: Would you like for us to install it? (y/n)?${COLOR_NC}" && read -p " " PROCEED
      echo ""
      case $PROCEED in
...
function install-directory-prompt() {
    if [[ -z $INSTALL_LOCATION ]]; then
        echo "No installation location was specified. Please provide the location where FOC is installed."
        while true; do
            [[ $NONINTERACTIVE == false ]] && printf "${COLOR_YELLOW}Do you wish to use the default location? ${FOC_INSTALL_DIR}? (y/n)${COLOR_NC}" && read -p " " PROCEED
            echo ""
            case $PROCEED in
                "" )
                echo "What would you like to do?";;
                ...
                break;;
                1 | false | [Nn]* )
                printf "Enter the desired installation location." && read -p " " FOC_INSTALL_DIR;
                export FOC_INSTALL_DIR;
                break;;
                * ) echo "Please type 'y' for yes or 'n' for no.";;
            esac
        done
    ...
    . ./scripts/.build_vars
    echo "FOC will be installed to: ${FOC_INSTALL_DIR}"
}

function previous-install-prompt() {
  if [[ -d $FOC_INSTALL_DIR ]]; then
    echo "FOC has already been installed into ${FOC_INSTALL_DIR}... It's suggested that you eosio_uninstall.sh before re-running this script."
    while true; do
...
function print_supported_linux_distros_and_exit() {
   echo "On Linux the FOC build script only supports Amazon, Centos, and Ubuntu."
...
function ensure-cmake() {
    ...
    if [[ ! -e "${CMAKE}" ]]; then
        ...
        && ./bootstrap --prefix=${FOC_INSTALL_DIR} \
        && make -j${JOBS} \
        && make install \
        && cd .. \
