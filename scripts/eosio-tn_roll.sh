#!/bin/bash
#
# eosio-tn_roll is used to have all of the instances of the FOC daemon on a host brought down
...
bash $FOC_HOME/scripts/eosio-tn_down.sh

cp $SDIR/$RD/$prog $RD/$prog

if [ $DD = "all" ]; then
    for FOC_RESTART_DATA_DIR in `ls -d var/lib/node_??`; do
        bash $FOC_HOME/scripts/eosio-tn_up.sh "$*"
    done
else
    bash $FOC_HOME/scripts/eosio-tn_up.sh "$*"
fi
