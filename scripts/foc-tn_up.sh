#!/bin/bash
#
# foc-tn_up is a helper script used to start a node that was previously stopped.
# It is not intended to be run stand-alone; it is a companion to the
# foc-tn_bounce.sh and foc-tn_roll.sh scripts.

connected="0"

rundir=programs/nodefoc
prog=nodefoc
...
datadir=var/lib/node_$FOC_NODE
...
relaunch() {
    echo "$rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/eosio/node_$FOC_NODE > $datadir/stdout.txt  2>> $datadir/$log "
    nohup $rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/eosio/node_$FOC_NODE > $datadir/stdout.txt  2>> $datadir/$log &
    pid=$!
    echo pid = $pid
