    ...
    for a in accounts:
        ...
        if not key in keys:
            if len(keys) >= args.max_user_keys:
                break
            keys[key] = True
            run(args.clfoc + 'wallet import --private-key ' + key)
...
def regProducers(b, e):
    for i in range(b, e):
        a = accounts[i]
        retry(args.clfoc + 'system regproducer ' + a['name'] + ' ' + a['pub'] + ' https://' + a['name'] + '.com' + '/' + a['pub'])

def listProducers():
    run(args.clfoc + 'system listproducers')
...
def claimRewards():
    ...
    for row in table['rows']:
        if row['unpaid_blocks'] and not row['last_claim_time']:
            times.append(getJsonOutput(args.clfoc + 'system claimrewards -j ' + row['owner'])['processed']['elapsed'])
    print('Elapsed time for claimrewards:', times)

def proxyVotes(b, e):
    vote(firstProducer, firstProducer + 1)
    proxy = accounts[firstProducer]['name']
    retry(args.clfoc + 'system regproxy ' + proxy)
    sleep(1.0)
    for i in range(b, e):
        voter = accounts[i]['name']
...
def resign(account, controller):
    ...
    sleep(1)
    run(args.clfoc + 'get account ' + account)
...
def randomTransfer(b, e):
    for j in range(20):
        ...
        run(args.clfoc + 'transfer -f ' + src + ' ' + dest + ' "0.0001 ' + args.symbol + '"' + ' || true')
...
def msigApproveReplaceSystem(proposer, proposalName):
    for i in range(firstProducer, firstProducer + numProducers):
        run(args.clfoc + 'multisig approve ' + proposer + ' ' + proposalName +
...
def produceNewAccounts():
    with open('newusers', 'w') as f:
        for i in range(120_000, 200_000):
            x = getOutput(args.clfoc + 'create key --to-console')
...
def stepKillAll():
    run('killall kfocd nodefoc || true')
    sleep(1.5)
def stepStartWallet():
    startWallet()
    importKeys()
...
parser.add_argument('-H', '--http-port', type=int, default=8000, metavar='', help='HTTP port for clfoc')
