...
namespace eosio { namespace chain {
   ...
   namespace detail {
      ...
      public:
         ...
         void skip( size_t s ) {
            ...
            FOC_ASSERT( status == 0, block_log_exception,
         ...
         bool read( char* d, size_t s ) {
            ...
            FOC_ASSERT( result == s, block_log_exception,
   ...
   void block_log::open(const fc::path& data_dir) {
      ...
      if (log_size) {
         ...
         FOC_ASSERT( my->version > 0, block_log_exception, "Block log was not setup properly" );
         FOC_ASSERT( is_supported_version(my->version), block_log_unsupported_version,
         ...
         if (my->version > 1){
            ...
            FOC_ASSERT(my->first_block_num > 0, block_log_exception, "Block log is malformed, first recorded block number is 0 but must be greater than or equal to 1");
         } else {
   ...
   uint64_t detail::block_log_impl::append(const signed_block_ptr& b) {
      try {
         FOC_ASSERT( genesis_written_to_block_log, block_log_append_fail, "Cannot append to block log until the genesis is first written" );
         ...
         FOC_ASSERT(index_file.tellp() == sizeof(uint64_t) * (b->block_num() - first_block_num),
   ...
   void block_log::reset( const chain_id_type& chain_id, uint32_t first_block_num ) {
      FOC_ASSERT( first_block_num > 1, block_log_exception,
   ...
   signed_block_ptr block_log::read_block_by_num(uint32_t block_num)const {
      try {
         ...
         if (pos != npos) {
            b = read_block(pos);
            FOC_ASSERT(b->block_num() == block_num, reversible_blocks_exception,
   ...
   block_id_type block_log::read_block_id_by_num(uint32_t block_num)const {
      try {
         ...
         if (pos != npos) {
            ...
            FOC_ASSERT(bh.block_num() == block_num, reversible_blocks_exception,
   ...
   fc::path block_log::repair_log( const fc::path& data_dir, uint32_t truncate_at_block ) {
      ...
      FOC_ASSERT( fc::is_directory(data_dir) && fc::is_regular_file(data_dir / "blocks.log"), block_log_not_found,
      ...
      FOC_ASSERT( blocks_dir_name.generic_string() != ".", block_log_exception, "Invalid path to blocks directory" );
      backup_dir = backup_dir / blocks_dir_name.generic_string().append("-").append( now );

      FOC_ASSERT( !fc::exists(backup_dir), block_log_backup_dir_exist,
      ...
      FOC_ASSERT( version > 0, block_log_exception, "Block log was not setup properly" );
      FOC_ASSERT( is_supported_version(version), block_log_unsupported_version,
                 "Unsupported version of block log. Block log version is ${version} while code supports version(s) [${min},${max}]",
      ...
      if (version != 1) {
         ...
         FOC_ASSERT(actual_totem == expected_totem, block_log_exception,
   ...
   fc::optional<ChainContext> detail::block_log_impl::extract_chain_context( const fc::path& data_dir, Lambda&& lambda ) {
      ...
      FOC_ASSERT( version >= block_log::min_supported_version && version <= block_log::max_supported_version, block_log_unsupported_version,
                  "Unsupported version of block log. Block log version is ${version} while code supports version(s) [${min},${max}]",
                  ("version", version)("min", block_log::min_supported_version)("max", block_log::max_supported_version) );
   ...
   chain_id_type block_log::extract_chain_id( const fc::path& data_dir ) {
      return *(detail::block_log_impl::extract_chain_context<chain_id_type>(data_dir, [](std::fstream& block_stream, uint32_t version, uint32_t first_block_num ) -> fc::optional<chain_id_type> {
         ...
         FOC_ASSERT( contains_chain_id(version, first_block_num), block_log_exception,
   ...
   uint32_t detail::reverse_iterator::open(const fc::path& block_file_name) {
      ...
      FOC_ASSERT( _file, block_log_exception, "Could not open Block log file at '${blocks_log}'", ("blocks_log", _block_file_name) );
      ...
      FOC_ASSERT( size == 1, block_log_exception, "Block log file at '${blocks_log}' could not be read.", ("file", _block_file_name) );
      FOC_ASSERT( block_log::is_supported_version(_version), block_log_unsupported_version,
      ...
      if (_version == 1) {
         ...
         FOC_ASSERT( size == 1, block_log_exception, "Block log file at '${blocks_log}' not formatted consistently with version ${v}.", ("file", _block_file_name)("v", _version) );
      }

      auto status = fseek(_file.get(), 0, SEEK_END);
      FOC_ASSERT( status == 0, block_log_exception, "Could not open Block log file at '${blocks_log}'. Returned status: ${status}", ("blocks_log", _block_file_name)("status", status) );

      _eof_position_in_file = ftell(_file.get());
      FOC_ASSERT( _eof_position_in_file > 0, block_log_exception, "Block log file at '${blocks_log}' could not be read.", ("blocks_log", _block_file_name) );
      ...
      if (block_pos >= _start_of_buffer_position) {
         ...
         FOC_ASSERT( status == 0, block_log_exception, "Could not seek in '${blocks_log}' to position: ${pos}. Returned status: ${status}", ("blocks_log", _block_file_name)("pos", blknum_offset_pos)("status", status) );
         auto size = fread((void*)&bnum, sizeof(bnum), 1, _file.get());
         FOC_ASSERT( size == 1, block_log_exception, "Could not read in '${blocks_log}' at position: ${pos}", ("blocks_log", _block_file_name)("pos", blknum_offset_pos) );
      }
   ...
   uint64_t detail::reverse_iterator::previous() {
      FOC_ASSERT( _current_position_in_file != block_log::npos,
                  block_log_exception,
      ...
      if (block_location_in_file == block_log::npos) {
         ...
         FOC_ASSERT( _blocks_found != _blocks_expected,
                    block_log_exception,
         ...
         FOC_ASSERT( _current_position_in_file < previous_position_in_file,
                     block_log_exception,
   ...
   void detail::reverse_iterator::update_buffer() {
      FOC_ASSERT( _current_position_in_file != block_log::npos, block_log_exception, "Block log file not setup properly" );
      ...
      FOC_ASSERT( status == 0, block_log_exception, "Could not seek in '${blocks_log}' to position: ${pos}. Returned status: ${status}", ("blocks_log", _block_file_name)("pos", _start_of_buffer_position)("status", status) );
      char* buf = _buffer_ptr.get();
      ...
      FOC_ASSERT( size == 1, block_log_exception, "blocks.log read fails" );
   }
   ...
   void detail::index_writer::prepare_buffer() {
      if (_file == nullptr) {
         ...
         FOC_ASSERT( _file, block_log_exception, "Could not open Block index file at '${blocks_index}'", ("blocks_index", _block_index_name) );
         ...
         FOC_ASSERT( status == 0, block_log_exception, "Could not allocate in '${blocks_index}' storage for all the blocks, size: ${size}. Returned status: ${status}", ("blocks_index", _block_index_name)("size", full_file_size)("status", status) );
   ...
   bool detail::index_writer::shift_buffer() {
      ...
      FOC_ASSERT( status == 0, block_log_exception, "Could not navigate in '${blocks_index}' file_location_start: ${loc}, _start_of_buffer_position: ${_start_of_buffer_position}. Returned status: ${status}", ("blocks_index", _block_index_name)("loc", file_location_start)("_start_of_buffer_position",_start_of_buffer_position)("status", status) );
      ...
      FOC_ASSERT( size == 1, block_log_exception, "Writing Block Index file '${file}' failed at location: ${loc}", ("file", _block_index_name)("loc", file_location_start) );
      update_buffer_position();
      return true;
   }
   ...
   void detail::index_writer::complete() {
      ...
      FOC_ASSERT(shifted, block_log_exception, "Failed to write buffer to '${blocks_index}'", ("blocks_index", _block_index_name) );
      FOC_ASSERT(_current_position == -1,
                 block_log_exception,
   ...
   bool block_log::trim_blocklog_front(const fc::path& block_dir, const fc::path& temp_dir, uint32_t truncate_at_block) {
      using namespace std;
      FOC_ASSERT( block_dir != temp_dir, block_log_exception, "block_dir and temp_dir need to be different directories" );
      ...
      FOC_ASSERT( status == 0, block_log_exception, "blocks.log seek failed" );
      ...
      for(uint64_t to_write_remaining = to_write; to_write_remaining > 0; to_write_remaining -= read_size) {
         ...
         FOC_ASSERT( num_read == 1, block_log_exception, "blocks.log read failed" );
   ...
   trim_data::trim_data(fc::path block_dir) {
      ...
      FOC_ASSERT( blk_in != nullptr, block_log_not_found, "cannot read file ${file}", ("file",block_file_name.string()) );
      ind_in = FC_FOPEN(index_file_name.generic_string().c_str(), "rb");
      FOC_ASSERT( ind_in != nullptr, block_log_not_found, "cannot read file ${file}", ("file",index_file_name.string()) );
      auto size = fread((void*)&version,sizeof(version), 1, blk_in);
      FOC_ASSERT( size == 1, block_log_unsupported_version, "invalid format for file ${file}", ("file",block_file_name.string()));
      ilog("block log version= ${version}",("version",version));
      FOC_ASSERT( block_log::is_supported_version(version), block_log_unsupported_version, "block log version ${v} is not supported", ("v",version));
      ...
      if (version == 1) {
         ...
         FOC_ASSERT(size == 1, block_log_exception, "invalid format for file ${file}",
         ...
         FOC_ASSERT(size == 1, block_log_exception,
      ...
      FOC_ASSERT( status == 0, block_log_exception, "cannot seek to ${file} end", ("file", index_file_name.string()) );
      ...
      FOC_ASSERT(start_of_blocks == first_block_pos, block_log_exception,
   ...
   uint64_t trim_data::block_index(uint32_t n) const {
      using namespace std;
      FOC_ASSERT( first_block <= n, block_log_exception,
      ...
      FOC_ASSERT( n <= last_block, block_log_exception,
   ...
   uint64_t trim_data::block_pos(uint32_t n) {
      ...
      FOC_ASSERT( status == 0, block_log_exception, "cannot seek to ${file} ${pos} from beginning of file for block ${b}", ("file", index_file_name.string())("pos", index_pos)("b",n) );
      const uint64_t pos = ftell(ind_in);
      FOC_ASSERT( pos == index_pos, block_log_exception, "cannot seek to ${file} entry for block ${b}", ("file", index_file_name.string())("b",n) );
      uint64_t block_n_pos;
      auto size = fread((void*)&block_n_pos, sizeof(block_n_pos), 1, ind_in);                   //filepos of block n
      FOC_ASSERT( size == 1, block_log_exception, "cannot read ${file} entry for block ${b}", ("file", index_file_name.string())("b",n) );
      ...
      FOC_ASSERT( status == 0, block_log_exception, "cannot seek to ${file} ${pos} from beginning of file", ("file", block_file_name.string())("pos", calc_blknum_pos) );
      const uint64_t block_offset_pos = ftell(blk_in);
      FOC_ASSERT( block_offset_pos == calc_blknum_pos, block_log_exception, "cannot seek to ${file} ${pos} from beginning of file", ("file", block_file_name.string())("pos", calc_blknum_pos) );
