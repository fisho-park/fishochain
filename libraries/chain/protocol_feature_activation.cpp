...
namespace eosio { namespace chain {
   ...
   void protocol_feature_activation::reflector_init() {
      ...
      FOC_ASSERT( protocol_features.size() > 0, ill_formed_protocol_feature_activation,
      ...
      for( const auto& d : protocol_features ) {
         auto res = s.insert( d );
         FOC_ASSERT( res.second, ill_formed_protocol_feature_activation,
   ...
   )
   {
      ...
      FOC_ASSERT( !enforce_disjoint || protocol_features.size() == expected_size,
