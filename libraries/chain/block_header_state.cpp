...
namespace eosio { namespace chain {
                                                         ...
                                                         uint16_t num_prev_blocks_to_confirm )const
   {
      ...
      if( when != block_timestamp_type() ) {
        FOC_ASSERT( when > header.timestamp, block_validate_exception, "next block must be in the future" );
      } else {
      ...
      if( itr != producer_to_last_produced.end() ) {
        FOC_ASSERT( itr->second < (block_num+1) - num_prev_blocks_to_confirm, producer_double_confirm,
         ...
         if ( detail::is_builtin_activated(prev_activated_protocol_features, pfs, builtin_protocol_feature_t::wtmsig_block_signatures) ) {
            ...
            for (const auto &p : new_producers->producers) {
               p.authority.visit([&downgraded_producers, &p](const auto& auth){
                  FOC_ASSERT(auth.keys.size() == 1 && auth.keys.front().weight == auth.threshold, producer_schedule_exception, "multisig block signing present before enabled!");
   ...
   {
      FOC_ASSERT( h.timestamp == timestamp, block_validate_exception, "timestamp mismatch" );
      FOC_ASSERT( h.previous == previous, unlinkable_block_exception, "previous mismatch" );
      FOC_ASSERT( h.confirmed == confirmed, block_validate_exception, "confirmed mismatch" );
      FOC_ASSERT( h.producer == producer, wrong_producer, "wrong producer specified" );
      FOC_ASSERT( h.schedule_version == active_schedule_version, producer_schedule_exception, "schedule_version in signed block is corrupted" );
      ...
      if( h.new_producers ) {
         FOC_ASSERT(!wtmsig_enabled, producer_schedule_exception, "Block header contains legacy producer schedule outdated by activation of WTMsig Block Signatures" );

         FOC_ASSERT( !was_pending_promoted, producer_schedule_exception, "cannot set pending producer schedule in the same block in which pending was promoted to active" );

         const auto& new_producers = *h.new_producers;
         FOC_ASSERT( new_producers.version == active_schedule.version + 1, producer_schedule_exception, "wrong producer schedule version specified" );
         FOC_ASSERT( prev_pending_schedule.schedule.producers.empty(), producer_schedule_exception,
      ...
      if ( exts.count(producer_schedule_change_extension::extension_id()) > 0 ) {
         FOC_ASSERT(wtmsig_enabled, producer_schedule_exception, "Block header producer_schedule_change_extension before activation of WTMsig Block Signatures" );
         FOC_ASSERT( !was_pending_promoted, producer_schedule_exception, "cannot set pending producer schedule in the same block in which pending was promoted to active" );
         ...
         FOC_ASSERT( new_producer_schedule.version == active_schedule.version + 1, producer_schedule_exception, "wrong producer schedule version specified" );
         FOC_ASSERT( prev_pending_schedule.schedule.producers.empty(), producer_schedule_exception,
                     "cannot set new pending producers until last pending is confirmed" );
   ...
   )&&
   {
      if( !additional_signatures.empty() ) {
         ...
         FOC_ASSERT(wtmsig_enabled, producer_schedule_exception, "Block contains multiple signatures before WTMsig block signatures are enabled" );
      }
   ...
   )&&
   {
      ...
      if( !result.additional_signatures.empty() ) {
         ...
         FOC_ASSERT(wtmsig_enabled, producer_schedule_exception, "Block was signed with multiple signatures before WTMsig block signatures are enabled" );
      }
   ...
   void block_header_state::sign( const signer_callback_type& signer ) {
      ...
      FOC_ASSERT(!sigs.empty(), no_block_signatures, "Signer returned no signatures");
   ...
   void block_header_state::verify_signee( )const {
      ...
      FOC_ASSERT(1 + additional_signatures.size() <= num_keys_in_authority, wrong_signing_key,
      ...
      for (const auto& s: additional_signatures) {
         ...
         FOC_ASSERT(res.second, wrong_signing_key, "block signed by same key twice", ("key", *res.first));
      }
      ...
      FOC_ASSERT(relevant_sig_count == keys.size(), wrong_signing_key,
                 "block signed by unexpected key",
                 ("signing_keys", keys)("authority", valid_block_signing_authority));

      FOC_ASSERT(is_satisfied, wrong_signing_key,
