...
namespace eosio { namespace chain {
   void additional_block_signatures_extension::reflector_init() {
      ...
      FOC_ASSERT( signatures.size() > 0, ill_formed_additional_block_signatures_extension,
      ...
      for( const auto& s : signatures ) {
         ...
         FOC_ASSERT( res.second, ill_formed_additional_block_signatures_extension,
   ...
   flat_multimap<uint16_t, block_extension> signed_block::validate_and_extract_extensions()const {
      ...
      for( size_t i = 0; i < block_extensions.size(); ++i ) {
         ...
         FOC_ASSERT( id >= id_type_lower_bound, invalid_block_extension,
         ...
         FOC_ASSERT( match, invalid_block_extension,
                     "Block extension with id type ${id} is not supported",
                     ("id", id)
         );

         if( match->enforce_unique ) {
            FOC_ASSERT( i == 0 || id > id_type_lower_bound, invalid_block_header_extension,
