...
struct controller_impl {
   ...
   void pop_block() {
      ...
      if( !prev ) {
         FOC_ASSERT( fork_db.root()->id == head->header.previous, block_validate_exception, "attempt to pop beyond last irreversible block" );
         prev = fork_db.root();
      }
      ...
      if ( read_mode == db_read_mode::SPECULATIVE ) {
         FOC_ASSERT( head->block, block_validate_exception, "attempting to pop a block that was sparsely loaded from a snapshot");
      }
   ...
   inline void set_activation_handler() {
      ...
      FOC_ASSERT( res.second, misc_exception, "attempting to set activation handler twice" );
   }
...
#define SET_APP_HANDLER( receiver, contract, action) \
   ...
   SET_APP_HANDLER( foc, foc, newaccount );
   SET_APP_HANDLER( foc, foc, setcode );
   SET_APP_HANDLER( foc, foc, setabi );
   SET_APP_HANDLER( foc, foc, updateauth );
   SET_APP_HANDLER( foc, foc, deleteauth );
   SET_APP_HANDLER( foc, foc, linkauth );
   SET_APP_HANDLER( foc, foc, unlinkauth );
/*
   SET_APP_HANDLER( foc, foc, postrecovery );
   SET_APP_HANDLER( foc, foc, passrecovery );
   SET_APP_HANDLER( foc, foc, vetorecovery );
*/

   SET_APP_HANDLER( foc, foc, canceldelay );
   }
   ...
   void log_irreversible() {
      FOC_ASSERT( fork_db.root(), fork_database_exception, "fork database not properly initialized" );
      ...
      if( log_head ) {
         FOC_ASSERT( root_id == blog.head_id(), fork_database_exception, "fork database root does not match block log head" );
      } else {
         FOC_ASSERT( fork_db.root()->block_num == lib_num, fork_database_exception,
   ...
   void replay(std::function<bool()> shutdown) {
      ...
      if( start_block_num <= blog_head->block_num() ) {
         ...
         if( pending_head->block_num < head->block_num || head->block_num < fork_db.root()->block_num ) {
            ...
            FOC_ASSERT( new_root, fork_database_exception, "unexpected error: could not find new LIB in fork database" );
   ...
   void startup(std::function<bool()> shutdown, const snapshot_reader_ptr& snapshot) {
      FOC_ASSERT( snapshot, snapshot_exception, "No snapshot reader provided" );
      ...
      try {
         ...
         if( blog.head() ) {
            ...
            FOC_ASSERT( lib_num > 0, snapshot_exception,
   ...
   void startup(std::function<bool()> shutdown, const genesis_state& genesis) {
      FOC_ASSERT( db.revision() < 1, database_exception, "This version of controller::startup only works with a fresh state database." );
      const auto& genesis_chain_id = genesis.compute_chain_id();
      FOC_ASSERT( genesis_chain_id == chain_id, chain_id_type_exception,
      ...
      if( blog.head() ) {
         FOC_ASSERT( blog.first_block_num() == 1, block_log_exception,
   ...
   void startup(std::function<bool()> shutdown) {
      FOC_ASSERT( db.revision() >= 1, database_exception, "This version of controller::startup does not work with a fresh state database." );
      FOC_ASSERT( fork_db.head(), fork_database_exception, "No existing fork database despite existing chain state. Replay required." );

      uint32_t lib_num = fork_db.root()->block_num;
      auto first_block_num = blog.first_block_num();
      if( blog.head() ) {
         FOC_ASSERT( first_block_num <= lib_num && lib_num <= blog.head()->block_num(),
                     block_log_exception,
   ...
   static auto validate_db_version( const chainbase::database& db ) {
      ...
      FOC_ASSERT(header_idx.begin() != header_idx.end(), bad_database_version_exception,
   ...
   void init(std::function<bool()> shutdown) {
...

      {
         ...
         FOC_ASSERT( state_chain_id == chain_id, chain_id_type_exception,
      ...
      FOC_ASSERT( db.revision() >= head->block_num, fork_database_exception,
      ...
      if( read_mode == db_read_mode::IRREVERSIBLE ) {
         ...
         FOC_ASSERT( itr == rbi.end() || itr->blocknum == lib_num + 1, reversible_blocks_exception,
         ...
         FOC_ASSERT( head->block_num <= last_block_num, reversible_blocks_exception,
         ...
         if( ritr != rbi.rend()
            ...
            FOC_ASSERT( rbitr != rbi.end(), reversible_blocks_exception, "pending head block not found in reversible blocks");
            auto rev_id = rbitr->get_block_id();
            FOC_ASSERT( rev_id == pending_head->id,
   ...
   void read_from_snapshot( const snapshot_reader_ptr& snapshot, uint32_t blog_start, uint32_t blog_end ) {
      ...
      { /// load and upgrade the block header state
         ...
         FOC_ASSERT( blog_start <= (snapshot_head_block + 1) && snapshot_head_block <= blog_end,
                     block_log_exception,
      ...
      controller_index_set::walk_indices([this, &snapshot, &header]( auto utils ){
         ...
         if (std::is_same<value_t, global_property_object>::value) {
            ...
            if (std::clamp(header.version, v2::minimum_version, v2::maximum_version) == header.version ) {
               ...
               FOC_ASSERT( genesis, snapshot_exception,
      ...
      FOC_ASSERT( gpo.chain_id == chain_id, chain_id_type_exception,
   ...
   void create_native_account( const fc::time_point& initial_timestamp, account_name name, const authority& owner, const authority& active, bool is_privileged = false ) {
      db.create<account_object>([&](auto& a) {
         ...
         if( name == config::system_account_name ) {
            // The initial eosio ABI value affects consensus; see  https://github.com/fisho-park/eos/issues/7794
   ...
   transaction_trace_ptr push_scheduled_transaction( const transaction_id_type& trxid, fc::time_point deadline, uint32_t billed_cpu_time_us, bool explicit_billed_cpu_time = false ) {
      ...
      FOC_ASSERT( itr != idx.end(), unknown_transaction_exception, "unknown transaction" );
   ...
   { try {
      ...
      FOC_ASSERT( gtrx.delay_until <= self.pending_block_time(), transaction_exception, "this transaction isn't ready",
   ...
   const transaction_receipt& push_receipt( const T& trx, transaction_receipt_header::status_enum status,
      ...
      FOC_ASSERT( net_usage_words*8 == net_usage, transaction_exception, "net_usage is not divisible by 8" );
   ...
   transaction_trace_ptr push_transaction( const transaction_metadata_ptr& trx,
                                           ...
                                           bool explicit_billed_cpu_time = false )
   {
      FOC_ASSERT(deadline != fc::time_point(), transaction_exception, "deadline cannot be uninitialized");
      ...
      try {
         ...
         if( !explicit_billed_cpu_time ) {
            fc::microseconds already_consumed_time( FOC_PERCENT(sig_cpu_usage.count(), conf.sig_cpu_bill_pct) );
   ...
   {
      FOC_ASSERT( !pending, block_validate_exception, "pending block already exists" );
      ...
      if (!self.skip_db_sessions(s)) {
         FOC_ASSERT( db.revision() == head->block_num, database_exception, "db revision is not on par with head block",
      ...
      if ( read_mode == db_read_mode::SPECULATIVE || pending->_block_status != controller::block_status::incomplete )
      {
         ...
         if( new_protocol_feature_activations.size() > 0 ) {
            ...
            for( const auto& feature_digest : new_protocol_feature_activations ) {
               ...
               if( res.second ) {
                  ...
                  FOC_ASSERT( !f.preactivation_required, protocol_feature_exception,
                              "attempted to activate protocol feature without prior required preactivation: ${digest}",
                              ("digest", feature_digest)
                  );
               } else {
                  FOC_ASSERT( !res.first->second, block_validate_exception,
         ...
         FOC_ASSERT( handled_all_preactivated_features, block_validate_exception,
         ...
         )
         {
            ...
            FOC_ASSERT( gpo.proposed_schedule.version == pbhs.active_schedule_version + 1,
   ...
   void finalize_block()
   {
      FOC_ASSERT( pending, block_validate_exception, "it is not valid to finalize when there is no pending block");
      FOC_ASSERT( pending->_block_stage.contains<building_block>(), block_validate_exception, "already called finalize_block");
      ...
      uint64_t CPU_TARGET = FOC_PERCENT(chain_config.max_block_cpu_usage, chain_config.target_block_cpu_usage_pct);
      resource_limits.set_block_parameters(
         ...
         {FOC_PERCENT(chain_config.max_block_net_usage, chain_config.target_block_net_usage_pct), chain_config.max_block_net_usage, config::block_size_average_window_ms / config::block_interval_ms, config::maximum_elastic_resource_multiplier, {99, 100}, {1000, 999}}
      );
   ...
   void commit_block( bool add_to_fork_db ) {
      ...
      try {
         FOC_ASSERT( pending->_block_stage.contains<completed_block>(), block_validate_exception,
         ...
         if( add_to_fork_db ) {
            ...
            head = fork_db.head();
            FOC_ASSERT( bsp == head, fork_database_exception, "committed block did not become the new head in fork database");
         }
      ...
      for( auto itr = new_protocol_features.begin(); itr != new_protocol_features.end(); ++itr ) {
         ...
         switch( status ) {
            case protocol_feature_set::recognized_t::unrecognized:
               FOC_THROW( protocol_feature_exception,
            ...
            break;
            default:
               FOC_THROW( protocol_feature_exception, "unexpected recognized_t status" );
            break;
         }

         FOC_ASSERT( currently_activated_protocol_features.find( f ) == currently_activated_protocol_features.end(),
         ...
         FOC_ASSERT( pfs.validate_dependencies( f, dependency_checker ), protocol_feature_exception,
...
#define FOC_REPORT(DESC,A,B) \
   if( A != B ) { \
      elog("${desc}: ${bv} != ${abv}", ("desc", DESC)("bv", A)("abv", B)); \
   }

      FOC_REPORT( "timestamp", b.timestamp, ab.timestamp )
      FOC_REPORT( "producer", b.producer, ab.producer )
      FOC_REPORT( "confirmed", b.confirmed, ab.confirmed )
      FOC_REPORT( "previous", b.previous, ab.previous )
      FOC_REPORT( "transaction_mroot", b.transaction_mroot, ab.transaction_mroot )
      FOC_REPORT( "action_mroot", b.action_mroot, ab.action_mroot )
      FOC_REPORT( "schedule_version", b.schedule_version, ab.schedule_version )
      FOC_REPORT( "new_producers", b.new_producers, ab.new_producers )
      FOC_REPORT( "header_extensions", b.header_extensions, ab.header_extensions )
...
#undef FOC_REPORT
      ...
      try {
         ...
         for( const auto& receipt : b->transactions ) {
            ...
            if( receipt.trx.contains<packed_transaction>() ) {
               ...
               FOC_ASSERT( false, block_validate_exception, "encountered unexpected receipt type" );
            }
         ...
         if( producer_block_id != ab._id ) {
            ...
            FOC_ASSERT( producer_block_id == ab._id, block_validate_exception, "Block ID does not match",
   ...
   std::future<block_state_ptr> create_block_state_future( const signed_block_ptr& b ) {
      FOC_ASSERT( b, block_validate_exception, "null block" );

      auto id = b->id();

      // no reason for a block_state if fork_db already knows about block
      auto existing = fork_db.get_block( id );
      FOC_ASSERT( !existing, fork_database_exception, "we already know about this block: ${id}", ("id", id) );

      auto prev = fork_db.get_block_header( b->previous );
      FOC_ASSERT( prev, unlinkable_block_exception,
      ...
      return async_thread_pool( thread_pool.get_executor(), [b, prev, control=this]() {
         ...
         FOC_ASSERT( b->transaction_mroot == trx_mroot, block_validate_exception,
                    ...
                    const forked_branch_callback& forked_branch_cb, const trx_meta_cache_lookup& trx_lookup )
   {
      ...
      FOC_ASSERT(!pending, block_validate_exception, "it is not valid to push a block when there is a pending block");
   ...
   void replay_push_block( const signed_block_ptr& b, controller::block_status s ) {
      ...
      try {
         FOC_ASSERT( b, block_validate_exception, "trying to push empty block" );
         FOC_ASSERT( (s == controller::block_status::irreversible || s == controller::block_status::validated),
         ...
         if( s == controller::block_status::irreversible ) {
            ...
            FOC_ASSERT( read_mode != db_read_mode::IRREVERSIBLE, block_validate_exception,
                            ...
                            const forked_branch_callback& forked_branch_cb, const trx_meta_cache_lookup& trx_lookup )
   {
      ...
      if( new_head->header.previous == head->id ) {
         ...
         if( branches.second.size() > 0 ) {
            ...
            FOC_ASSERT( self.head_block_id() == branches.second.back()->header.previous, fork_database_exception,
   ...
   void check_actor_list( const flat_set<account_name>& actors )const {
      ...
      if( conf.actor_whitelist.size() > 0 ) {
         ...
         FOC_ASSERT( is_subset,  actor_whitelist_exception,
         ...
         FOC_ASSERT( !intersects, actor_blacklist_exception,
   ...
   void check_contract_list( account_name code )const {
      if( conf.contract_whitelist.size() > 0 ) {
         FOC_ASSERT( conf.contract_whitelist.find( code ) != conf.contract_whitelist.end(),
                     contract_whitelist_exception,
                     "account '${code}' is not on the contract whitelist", ("code", code)
                   );
      } else if( conf.contract_blacklist.size() > 0 ) {
         FOC_ASSERT( conf.contract_blacklist.find( code ) == conf.contract_blacklist.end(),
   ...
   void check_action_list( account_name code, action_name action )const {
      if( conf.action_blacklist.size() > 0 ) {
         FOC_ASSERT( conf.action_blacklist.find( std::make_pair(code, action) ) == conf.action_blacklist.end(),
   ...
   void check_key_list( const public_key_type& key )const {
      if( conf.key_blacklist.size() > 0 ) {
         FOC_ASSERT( conf.key_blacklist.find( key ) == conf.key_blacklist.end(),
   ...
   void validate_tapos( const transaction& trx )const {
      ...
      FOC_ASSERT(trx.verify_reference_block(tapos_block_summary.block_id), invalid_ref_block_exception,
...
void controller::preactivate_feature( const digest_type& feature_digest ) {
   ...
   switch( status ) {
      case protocol_feature_set::recognized_t::unrecognized:
         if( is_producing_block() ) {
            FOC_THROW( subjective_block_production_exception,
                       "protocol feature with digest '${digest}' is unrecognized", ("digest", feature_digest) );
         } else {
            FOC_THROW( protocol_feature_bad_block_exception,
                       "protocol feature with digest '${digest}' is unrecognized", ("digest", feature_digest) );
         }
      break;
      case protocol_feature_set::recognized_t::disabled:
      ...
      default:
         if( is_producing_block() ) {
            FOC_THROW( subjective_block_production_exception, "unexpected recognized_t status" );
         } else {
            FOC_THROW( protocol_feature_bad_block_exception, "unexpected recognized_t status" );
         }
      break;
   }
   ...
   // Even if they don't recognize the feature, the producer could change their nodefoc code to treat it like an
   ...
   FOC_ASSERT( !is_protocol_feature_activated( feature_digest ),
               protocol_feature_exception,
   ...
   FOC_ASSERT( std::find( pso.preactivated_protocol_features.begin(),
   ...
   FOC_ASSERT( pfs.validate_dependencies( feature_digest, dependency_checker ),
               protocol_feature_exception,
...
void controller::start_block( block_timestamp_type when, uint16_t confirm_block_count )
{
   ...
   FOC_ASSERT( !my->pending, block_validate_exception, "pending block already exists" );
...
transaction_trace_ptr controller::push_transaction( const transaction_metadata_ptr& trx, fc::time_point deadline, uint32_t billed_cpu_time_us ) {
   validate_db_available_size();
   FOC_ASSERT( !in_immutable_mode(), transaction_type_exception, "push transaction not allowed in read-only mode" );
   FOC_ASSERT( trx && !trx->implicit && !trx->scheduled, transaction_type_exception, "Implicit/Scheduled transaction not allowed" );
...
{
   FOC_ASSERT( !in_immutable_mode(), transaction_type_exception, "push scheduled transaction not allowed in read-only mode" );
   validate_db_available_size();
...
void controller::set_action_blacklist( const flat_set< pair<account_name, action_name> >& new_action_blacklist ) {
   for (auto& act: new_action_blacklist) {
      FOC_ASSERT(act.first != account_name(), name_type_exception, "Action blacklist - contract name should not be empty");
      FOC_ASSERT(act.second != action_name(), action_type_exception, "Action blacklist - action name should not be empty");
   }
...
time_point controller::pending_block_time()const {
   FOC_ASSERT( my->pending, block_validate_exception, "no pending block" );
...
block_id_type controller::get_block_id_for_num( uint32_t block_num )const { try {
   ...
   FOC_ASSERT( BOOST_LIKELY( id != block_id_type() ), unknown_block_exception,
...
void controller::write_snapshot( const snapshot_writer_ptr& snapshot ) const {
   FOC_ASSERT( !my->pending, block_validate_exception, "cannot take a consistent snapshot with a pending block" );
...
void controller::validate_expiration( const transaction& trx )const { try {
   ...
   FOC_ASSERT( time_point(trx.expiration) >= pending_block_time(),
               expired_tx_exception,
               "transaction has expired, "
               "expiration is ${trx.expiration} and pending block time is ${pending_block_time}",
               ("trx.expiration",trx.expiration)("pending_block_time",pending_block_time()));
   FOC_ASSERT( time_point(trx.expiration) <= pending_block_time() + fc::seconds(chain_configuration.max_transaction_lifetime),
...
void controller::validate_db_available_size() const {
   ...
   FOC_ASSERT(free >= guard, database_guard_exception, "database free: ${f}, guard size: ${g}", ("f", free)("g",guard));
}
...
void controller::validate_reversible_available_size() const {
   ...
   FOC_ASSERT(free >= guard, reversible_guard_exception, "reversible free: ${f}, guard size: ${g}", ("f", free)("g",guard));
}
...
void controller::set_greylist_limit( uint32_t limit ) {
   FOC_ASSERT( 0 < limit && limit <= chain::config::maximum_elastic_resource_multiplier,
               misc_exception,
