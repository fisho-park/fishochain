...
const static name system_account_name    { N(foc) };
const static name null_account_name      { N(foc.null) };
const static name producers_account_name { N(foc.prods) };
...
const static name eosio_auth_scope       { N(foc.auth) };
const static name eosio_all_scope        { N(foc.all) };

const static name active_name     { N(active) };
const static name owner_name      { N(owner) };
const static name eosio_any_name  { N(foc.any) };
const static name eosio_code_name { N(foc.code) };
...
constexpr uint64_t FOC_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / eosio::chain::config::percent_100;
}

template<typename Number>
Number FOC_PERCENT_CEIL(Number value, uint32_t percentage) {
