   ...
   struct injector_utils {
      ...
      static void add_import(Module& module, const char* func_name, int32_t& index ) {
         if (module.functions.imports.size() == 0 || registered_injected.find(func_name) == registered_injected.end() ) {
            ...
            decltype(module.functions.imports) new_import = { {{func_type_index}, FOC_INJECTED_MODULE_NAME, std::move(func_name)} };
      ...
      public:
         ...
         void inject() {
            ...
            for ( auto& fd : _module->functions.defs ) {
               wasm_ops::FOC_OperatorDecoderStream<std::conditional_t<full_injection, pre_op_full_injectors, pre_op_injectors>> pre_decoder(fd.code);
            ...
            for ( auto& fd : _module->functions.defs ) {
               wasm_ops::FOC_OperatorDecoderStream<std::conditional_t<full_injection, post_op_full_injectors, post_op_injectors>> post_decoder(fd.code);
