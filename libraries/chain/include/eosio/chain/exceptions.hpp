...
#define FOC_ASSERT( expr, exc_type, FORMAT, ... )                \
   FC_MULTILINE_MACRO_BEGIN                                           \
   if( !(expr) )                                                      \
      FC_THROW_EXCEPTION( exc_type, FORMAT, __VA_ARGS__ );            \
   FC_MULTILINE_MACRO_END

#define FOC_THROW( exc_type, FORMAT, ... ) \
 ...
 */
#define FOC_RETHROW_EXCEPTIONS(exception_type, FORMAT, ... ) \
 ...
 * This macro will rethrow the exception as the specified "exception_type"
 */
#define FOC_CAPTURE_AND_RETHROW( exception_type, ... ) \
   catch( const std::bad_alloc& ) {\
      throw;\
   } catch( const boost::interprocess::bad_alloc& ) {\
      throw;\
...
#define FOC_RECODE_EXC( cause_type, effect_type ) \
