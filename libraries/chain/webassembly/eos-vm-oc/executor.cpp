...
#error FOC VM OC is not compatible with Clang ShadowCallStack
#endif
#endif
...
static void segv_handler(int sig, siginfo_t* info, void* ctx)  {
   ...
   if((uintptr_t)info->si_addr >= cb_in_main_segment->execution_thread_code_start &&
      (uintptr_t)info->si_addr < cb_in_main_segment->execution_thread_code_start+cb_in_main_segment->execution_thread_code_length)
         siglongjmp(*cb_in_main_segment->jmp, FOCVMOC_EXIT_CHECKTIME_FAIL);
   ...
   if((uintptr_t)info->si_addr >= cb_in_main_segment->execution_thread_memory_start &&
      (uintptr_t)info->si_addr < cb_in_main_segment->execution_thread_memory_start+cb_in_main_segment->execution_thread_memory_length)
         siglongjmp(*cb_in_main_segment->jmp, FOCVMOC_EXIT_SEGV);
...
static intrinsic grow_memory_intrinsic FOCVMOC_INTRINSIC_INIT_PRIORITY("eosvmoc_internal.grow_memory", IR::FunctionType::get(IR::ResultType::i32,{IR::ValueType::i32,IR::ValueType::i32}),
...
static void eosio_exit(int32_t code) {
   siglongjmp(*eos_vm_oc_get_jmp_buf(), FOCVMOC_EXIT_CLEAN_EXIT);
   __builtin_unreachable();
}
...
#define DEFINE_FOCVMOC_TRAP_INTRINSIC(module,name) \
	void name(); \
	static intrinsic name##Function FOCVMOC_INTRINSIC_INIT_PRIORITY(#module "." #name,IR::FunctionType::get(),(void*)&name, \
     boost::hana::index_if(intrinsic_table, ::boost::hana::equal.to(BOOST_HANA_STRING(#module "." #name))).value() \
   ); \
	void name()

DEFINE_FOCVMOC_TRAP_INTRINSIC(eosvmoc_internal,depth_assert) {
   throw_internal_exception("Exceeded call depth maximum");
}

DEFINE_FOCVMOC_TRAP_INTRINSIC(eosvmoc_internal,div0_or_overflow) {
   throw_internal_exception("Division by 0 or integer overflow trapped");
}

DEFINE_FOCVMOC_TRAP_INTRINSIC(eosvmoc_internal,indirect_call_mismatch) {
   throw_internal_exception("Indirect call function type mismatch");
}

DEFINE_FOCVMOC_TRAP_INTRINSIC(eosvmoc_internal,indirect_call_oob) {
   throw_internal_exception("Indirect call index out of bounds");
}

DEFINE_FOCVMOC_TRAP_INTRINSIC(eosvmoc_internal,unreachable) {
...
executor::executor(const code_cache_base& cc) {
   ...
   if(arch_prctl(ARCH_GET_GS, &current_gs) || current_gs)
      wlog("x86_64 GS register is not set as expected. FOC VM OC may not run correctly on this platform");
...
void executor::execute(const code_descriptor& code, const memory& mem, apply_context& context) {
   ...
   switch(sigsetjmp(*cb->jmp, 0)) {
      ...
      case FOCVMOC_EXIT_CHECKTIME_FAIL:
         context.trx_context.checktime();
         break;
      case FOCVMOC_EXIT_SEGV:
         FOC_ASSERT(false, wasm_execution_error, "access violation");
         break;
      case FOCVMOC_EXIT_EXCEPTION: //exception
