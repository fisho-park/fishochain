...
static constexpr uint64_t header_id = 0x32434f4d56534f45ULL; //"FOCVMOC2" little endian
...
{
   FC_ASSERT(_threads, "FOC VM OC requires at least 1 compile thread");
      ...
      if(_outstanding_compiles_and_poison[result.code] == false) {
         result.result.visit(overloaded {
            ...
            [&](const compilation_result_unknownfailure&) {
               wlog("code ${c} failed to tier-up with FOC VM OC", ("c", result.code.code_id));
   ...
   if(_outstanding_compiles_and_poison.size()) {
      ...
      while(count_processed && _queued_compiles.size()) {
         ...
         if(codeobject) {
            ...
            FC_ASSERT(write_message_with_fds(_compile_monitor_write_socket, compile_wasm_message{ *nextup }, fds_to_pass), "FOC VM failed to communicate to OOP manager");
            --count_processed;
         }
...
code_cache_sync::~code_cache_sync() {
   ...
   auto [success, message, fds] = read_message_with_fds(_compile_monitor_read_socket);
   if(success)
      elog("unexpected response from FOC VM OC compile monitor during shutdown");
}
...
const code_descriptor* const code_cache_sync::get_descriptor_for_code_sync(const digest_type& code_id, const uint8_t& vm_version) {
   ...
   auto [success, message, fds] = read_message_with_fds(_compile_monitor_read_socket);
   FOC_ASSERT(success, wasm_execution_error, "failed to read response from monitor process");
   FOC_ASSERT(message.contains<wasm_compilation_result_message>(), wasm_execution_error, "unexpected response from monitor process");

   wasm_compilation_result_message result = message.get<wasm_compilation_result_message>();
   FOC_ASSERT(result.result.contains<code_descriptor>(), wasm_execution_error, "failed to compile wasm");
   ...
   _cache_file_path(data_dir/"code_cache.bin")
{
   ...
   if(!bfs::exists(_cache_file_path)) {
      FOC_ASSERT(eosvmoc_config.cache_size >= allocator_t::get_min_size(total_header_size), database_exception, "configured code cache size is too small");
      std::ofstream ofs(_cache_file_path.generic_string(), std::ofstream::trunc);
      FOC_ASSERT(ofs.good(), database_exception, "unable to create FOC VM Optimized Compiler code cache");
   ...
   code_cache_header cache_header;
   {
      ...
      FOC_ASSERT(!hs.fail(), bad_database_version_exception, "failed to read code cache header");
      memcpy((char*)&cache_header, header_buff + header_offset, sizeof(cache_header));
   }

   FOC_ASSERT(cache_header.id == header_id, bad_database_version_exception, "existing FOC VM OC code cache not compatible with this version");
   FOC_ASSERT(!cache_header.dirty, database_exception, "code cache is dirty");
   ...
   FOC_ASSERT(_cache_fd >= 0, database_exception, "failure to open code cache");
   ...
   FOC_ASSERT(code_mapping != MAP_FAILED, database_exception, "failure to mmap code cache");
   ...
   if(cache_header.serialized_descriptor_index) {
      ...
      ilog("FOC VM Optimized Compiler code cache loaded with ${c} entries; ${f} of ${t} bytes free", ("c", number_entries)("f", allocator->get_free_memory())("t", allocator->get_size()));
   }
