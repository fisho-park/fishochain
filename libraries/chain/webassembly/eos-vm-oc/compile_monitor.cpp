...
struct compile_monitor_session {
   compile_monitor_session(boost::asio::io_context& context, local::datagram_protocol::socket&& n, wrapped_fd&& c, wrapped_fd& t) :
      _ctx(context),
      _nodefoc_instance_socket(std::move(n)),
   ...
   void read_message_from_nodefoc() {
      _nodefoc_instance_socket.async_wait(local::datagram_protocol::socket::wait_read, [this](auto ec) {
         if(ec) {
            connection_dead_signal();
            return;
         }
         auto [success, message, fds] = read_message_with_fds(_nodefoc_instance_socket);
         if(!success) {
            connection_dead_signal();
            return;
         }
   ...
   void kick_compile_off(const code_tuple& code_id, wrapped_fd&& wasm_code) {
      ...
      if(write_message_with_fds(_trampoline_socket, trampoline_compile_request, fds_pass_to_trampoline) == false) {
         ...
         write_message_with_fds(_nodefoc_instance_socket, reply);
         return;
      }
...
private:
   ...
   local::datagram_protocol::socket _nodefoc_instance_socket;
...
struct compile_monitor {
   compile_monitor(boost::asio::io_context& ctx, local::datagram_protocol::socket&& n, wrapped_fd&& t) : _nodefoc_socket(std::move(n)), _trampoline_socket(std::move(t)) {
      //the only duty of compile_monitor is to create a compile_monitor_session when a code_cache instance
      // in nodefoc wants one
      wait_for_new_incomming_session(ctx);
   }

   void wait_for_new_incomming_session(boost::asio::io_context& ctx) {
      _nodefoc_socket.async_wait(boost::asio::local::datagram_protocol::socket::wait_read, [this, &ctx](auto ec) {
         if(ec) {
            ctx.stop();
            return;
         }
         auto [success, message, fds] = read_message_with_fds(_nodefoc_socket);
         if(!success) {   //failure reading indicates that nodefoc has shut down
            ctx.stop();
            return;
         }
         ...
         try {
            ...
            write_message_with_fds(_nodefoc_socket, initalize_response_message());
         }
         catch(const fc::exception& e) {
            write_message_with_fds(_nodefoc_socket, initalize_response_message{e.what()});
         }
         catch(...) {
            write_message_with_fds(_nodefoc_socket, initalize_response_message{"Failed to create compile process"});
         }
   ...
   local::datagram_protocol::socket _nodefoc_socket;
   wrapped_fd _trampoline_socket;

   std::list<compile_monitor_session> _compile_sessions;
};

void launch_compile_monitor(int nodefoc_fd) {
...

   {
      ...
      boost::asio::local::datagram_protocol::socket nodefoc_socket(ctx);
      nodefoc_socket.assign(boost::asio::local::datagram_protocol(), nodefoc_fd);
      wrapped_fd trampoline_socket(socks[0]);
      compile_monitor monitor(ctx, std::move(nodefoc_socket), std::move(trampoline_socket));
      ctx.run();
      if(monitor._compile_sessions.size())
         std::cerr << "ERROR: FOC VM OC compiler monitor exiting with active sessions" << std::endl;
   }
...
wrapped_fd get_connection_to_compile_monitor(int cache_fd) {
   FC_ASSERT(the_compile_monitor_trampoline.compile_manager_pid >= 0, "FOC VM oop connection doesn't look active");
   ...
   FOC_ASSERT(success, misc_exception, "failed to read response from monitor process");
   FOC_ASSERT(message.contains<initalize_response_message>(), misc_exception, "unexpected response from monitor process");
   FOC_ASSERT(!message.get<initalize_response_message>().error_message, misc_exception, "Error message from monitor process: ${e}", ("e", *message.get<initalize_response_message>().error_message));
