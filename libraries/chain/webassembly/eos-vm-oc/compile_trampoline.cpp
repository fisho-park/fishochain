...
void run_compile_trampoline(int fd) {
   ...
   while(true) {
      ...
      if(!message.contains<compile_wasm_message>() || fds.size() != 2) {
         std::cerr << "FOC VM OC compile trampoline got unexpected message; ignoring" << std::endl;
         continue;
      }
      ...
      if(pid == 0) {
         ...
         std::cerr << "FOC VM OC compile trampoline failed to spawn compile task" << std::endl;
   }
