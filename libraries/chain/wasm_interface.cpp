   ...
   void wasm_interface::apply( const digest_type& code_hash, const uint8_t& vm_type, const uint8_t& vm_version, apply_context& context ) {
      ...
      if(my->eosvmoc) {
         ...
         try {
            ...
            //swallow errors here, if FOC VM OC has gone in to the weeds we shouldn't bail: continue to try and run baseline
            //In the future, consider moving bits of FOC VM that can fire exceptions and such out of this call path
            static bool once_is_enough;
            if(!once_is_enough)
               elog("FOC VM OC has encountered an unexpected failure");
            once_is_enough = true;
         }
         if(cd) {
   ...
   public:
      ...
      :context(ctx)
      {
         if( context.is_context_free() )
            FOC_ASSERT( context_free, unaccessible_api, "only context free api's can be used in this context" );
      }
...
class context_free_api : public context_aware_api {
   public:
      ...
      :context_aware_api(ctx, true) {
         ...
         FOC_ASSERT( context.is_context_free(), unaccessible_api, "this API may only be called from context_free apply" );
      }
...
class privileged_api : public context_aware_api {
   public:
      ...
      {
         FOC_ASSERT( context.is_privileged(), unaccessible_api, "${code} does not have permission to call this API", ("code",context.get_receiver()) );
      }
      ...
      void activate_feature( int64_t feature_name ) {
         FOC_ASSERT( false, unsupported_feature, "Unsupported Hardfork Detected" );
      }
      ...
      void set_resource_limits( account_name account, int64_t ram_bytes, int64_t net_weight, int64_t cpu_weight) {
         FOC_ASSERT(ram_bytes >= -1, wasm_execution_error, "invalid value for ram resource limit expected [-1,INT64_MAX]");
         FOC_ASSERT(net_weight >= -1, wasm_execution_error, "invalid value for net resource weight expected [-1,INT64_MAX]");
         FOC_ASSERT(cpu_weight >= -1, wasm_execution_error, "invalid value for cpu resource weight expected [-1,INT64_MAX]");
      ...
      int64_t set_proposed_producers_common( vector<producer_authority> && producers, bool validate_keys ) {
         FOC_ASSERT(producers.size() <= config::max_producers, wasm_execution_error, "Producer schedule exceeds the maximum producer count for this chain");
         FOC_ASSERT( producers.size() > 0
         ...
         for (const auto& p: producers) {
            FOC_ASSERT( context.is_account(p.producer_name), wasm_execution_error, "producer schedule includes a nonexisting account" );
            ...
            p.authority.visit([&p, num_supported_key_types, validate_keys](const auto& a) {
               ...
               for (const auto& kw: a.keys ) {
                  FOC_ASSERT( kw.key.which() < num_supported_key_types, unactivated_key_type,
                              "Unactivated key type used in proposed producer schedule");

                  if( validate_keys ) {
                     FOC_ASSERT( kw.key.valid(), wasm_execution_error, "producer schedule includes an invalid key" );
                  }
               ...
               FOC_ASSERT( a.keys.size() == unique_keys.size(), wasm_execution_error, "producer schedule includes a duplicated key for ${account}", ("account", p.producer_name));
               FOC_ASSERT( a.threshold > 0, wasm_execution_error, "producer schedule includes an authority with a threshold of 0 for ${account}", ("account", p.producer_name));
               FOC_ASSERT( sum_weights >= a.threshold, wasm_execution_error, "producer schedule includes an unsatisfiable authority for ${account}", ("account", p.producer_name));
            });


            unique_producers.insert(p.producer_name);
         }
         FOC_ASSERT( producers.size() == unique_producers.size(), wasm_execution_error, "duplicate producer name in producer schedule" );
      ...
      int64_t set_proposed_producers_ex( uint64_t packed_producer_format, array_ptr<char> packed_producer_schedule, uint32_t datalen ) {
         if (packed_producer_format == 0) {
            ...
            FOC_THROW(wasm_execution_error, "Producer schedule is in an unknown format!");
         }
      }
...
class crypto_api : public context_aware_api {
   public:
      ...
      void assert_recover_key( const fc::sha256& digest,
         ...
         FOC_ASSERT(s.which() < context.db.get<protocol_state_object>().num_supported_key_types, unactivated_signature_type,
           "Unactivated signature type used during assert_recover_key");
         FOC_ASSERT(p.which() < context.db.get<protocol_state_object>().num_supported_key_types, unactivated_key_type,
           "Unactivated key type used when creating assert_recover_key");

         if(context.control.is_producing_block())
            FOC_ASSERT(s.variable_size() <= context.control.configured_subjective_signature_length_limit(),
                       sig_variable_size_limit_exception, "signature variable length component size greater than subjective maximum");

         auto check = fc::crypto::public_key( s, digest, false );
         FOC_ASSERT( check == p, crypto_api_exception, "Error expected key different than recovered key" );
      }
      ...
      int recover_key( const fc::sha256& digest,
         ...
         if (s.which() >= config::genesis_num_supported_key_types ) {
            FOC_ASSERT(publen >= 33, wasm_execution_error,
      ...
      void assert_sha256(array_ptr<char> data, uint32_t datalen, const fc::sha256& hash_val) {
         ...
         FOC_ASSERT( result == hash_val, crypto_api_exception, "hash mismatch" );
      }
...
class permission_api : public context_aware_api {
   public:
      ...
      {
         FOC_ASSERT( delay_us <= static_cast<uint64_t>(std::numeric_limits<int64_t>::max()),
      ...
      int64_t get_account_creation_time( account_name account ) {
         ...
         FOC_ASSERT( acct != nullptr, action_validate_exception,
...
public:
   ...
   void abort() {
      FOC_ASSERT( false, abort_called, "abort() called");
   }
   ...
   void eosio_assert( bool condition, null_terminated_ptr msg ) {
      if( BOOST_UNLIKELY( !condition ) ) {
         ...
         FOC_THROW( eosio_assert_message_exception, "assertion failure with message: ${s}", ("s",message) );
      }
   }
...
#define DB_API_METHOD_WRAPPERS_FLOAT_SECONDARY(IDX, TYPE)\
      int db_##IDX##_store( uint64_t scope, uint64_t table, uint64_t payer, uint64_t id, const TYPE& secondary ) {\
         FOC_ASSERT( !softfloat_api::is_nan( secondary ), transaction_exception, "NaN is not an allowed value for a secondary key" );\
         return context.IDX.store( scope, table, account_name(payer), id, secondary );\
      }\
      void db_##IDX##_update( int iterator, uint64_t payer, const TYPE& secondary ) {\
         FOC_ASSERT( !softfloat_api::is_nan( secondary ), transaction_exception, "NaN is not an allowed value for a secondary key" );\
...
class memory_api : public context_aware_api {
   public:
      ...
      char* memcpy( array_ptr<char> dest, array_ptr<const char> src, uint32_t length) {
         FOC_ASSERT((size_t)(std::abs((ptrdiff_t)dest.value - (ptrdiff_t)src.value)) >= length,
...
class transaction_api : public context_aware_api {
   public:
      ...
      void send_inline( array_ptr<char> data, uint32_t data_len ) {
         ...
         FOC_ASSERT( data_len < context.control.get_global_properties().configuration.max_inline_action_size, inline_action_too_big,
...
class compiler_builtins : public context_aware_api {
   public:
      ...
      void __divti3(__int128& ret, uint64_t la, uint64_t ha, uint64_t lb, uint64_t hb) {
         ...
         FOC_ASSERT(rhs != 0, arithmetic_exception, "divide by zero");
