...
namespace eosio { namespace chain {
   ...
   void transaction_context::disallow_transaction_extensions( const char* error_msg )const {
      if( control.is_producing_block() ) {
         FOC_THROW( subjective_block_production_exception, error_msg );
      } else {
         FOC_THROW( disallowed_transaction_extensions_bad_block_exception, error_msg );
      }
   }

   void transaction_context::init(uint64_t initial_net_usage)
   {
      FOC_ASSERT( !is_initialized, transaction_exception, "cannot initialize twice" );
   ...
   void transaction_context::exec() {
      FOC_ASSERT( is_initialized, transaction_exception, "must first initialize" );
      ...
      if (!control.skip_trx_checks()) {
         if( BOOST_UNLIKELY(net_usage > eager_net_limit) ) {
            if ( net_limit_due_to_block ) {
               FOC_THROW( block_net_usage_exceeded,
               ...
               FOC_THROW( greylist_net_usage_exceeded,
                          "greylisted transaction net usage is too high: ${net_usage} > ${net_limit}",
                          ("net_usage", net_usage)("net_limit", eager_net_limit) );
            } else {
               FOC_THROW( tx_net_usage_exceeded,
   ...
   void transaction_context::checktime()const {
      ...
      if( explicit_billed_cpu_time || deadline_exception_code == deadline_exception::code_value ) {
         FOC_THROW( deadline_exception, "deadline exceeded ${billing_timer}us",
                     ("billing_timer", now - pseudo_start)("now", now)("deadline", _deadline)("start", start) );
      } else if( deadline_exception_code == block_cpu_usage_exceeded::code_value ) {
         FOC_THROW( block_cpu_usage_exceeded,
   ...
   void transaction_context::validate_cpu_usage_to_bill( int64_t billed_us, bool check_minimum )const {
      if (!control.skip_trx_checks()) {
         if( check_minimum ) {
            const auto& cfg = control.get_global_properties().configuration;
            FOC_ASSERT( billed_us >= cfg.min_transaction_cpu_usage, transaction_exception,
         ...
         if( billing_timer_exception_code == block_cpu_usage_exceeded::code_value ) {
            FOC_ASSERT( billed_us <= objective_duration_limit.count(),
   ...
   action_trace& transaction_context::get_action_trace( uint32_t action_ordinal ) {
      FOC_ASSERT( 0 < action_ordinal && action_ordinal <= trace->action_traces.size() ,
   ...
   void transaction_context::validate_referenced_accounts( const transaction& trx, bool enforce_actor_whitelist_blacklist )const {
      ...
      for( const auto& a : trx.context_free_actions ) {
         ...
         FOC_ASSERT( code != nullptr, transaction_exception,
                     "action's code account '${account}' does not exist", ("account", a.account) );
         FOC_ASSERT( a.authorization.size() == 0, transaction_exception,
      ...
      for( const auto& a : trx.actions ) {
         ...
         for( const auto& auth : a.authorization ) {
            ...
            FOC_ASSERT( actor  != nullptr, transaction_exception,
                        "action's authorizing actor '${account}' does not exist", ("account", auth.actor) );
            FOC_ASSERT( auth_manager.find_permission(auth) != nullptr, transaction_exception,
      ...
      FOC_ASSERT( one_auth, tx_no_auths, "transaction must have at least one authorization" );
