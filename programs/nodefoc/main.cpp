...
{
   try {
      app().set_version(eosio::nodefoc::config::version);
      app().set_version_string(eosio::version::version_client());
      app().set_full_version_string(eosio::version::version_full());

      auto root = fc::app_path();
      app().set_default_data_dir(root / "eosio" / nodefoc::config::node_executable_name / "data" );
      app().set_default_config_dir(root / "eosio" / nodefoc::config::node_executable_name / "config" );
      ...
      ilog( "${name} version ${ver} ${fv}",
            ("name", nodefoc::config::node_executable_name)("ver", app().version_string())
            ("fv", app().version_string() == app().full_version_string() ? "" : app().full_version_string()) );
      ilog("${name} using configuration file ${c}", ("name", nodefoc::config::node_executable_name)("c", app().full_config_file_path().string()));
      ilog("${name} data directory is ${d}", ("name", nodefoc::config::node_executable_name)("d", app().data_dir().string()));
      app().startup();
   ...
   ilog("${name} successfully exiting", ("name", nodefoc::config::node_executable_name));
   return SUCCESS;
}
