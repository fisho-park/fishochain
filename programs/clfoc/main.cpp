/**
  ...
  @section intro Introduction to clfoc

  `clfoc` is a command line tool that interfaces with the REST api exposed by @ref nodefoc. In order to use `clfoc` you will need to
  have a local copy of `nodefoc` running and configured to load the 'eosio::chain_api_plugin'.

   clfoc contains documentation for all of its commands. For a list of all commands known to clfoc, simply run it with no arguments:
```
$ ./clfoc
Command Line Interface to FOC Client
Usage: programs/clfoc/clfoc [OPTIONS] SUBCOMMAND

Options:
  -h,--help                   Print this help message and exit
  -u,--url TEXT=http://localhost:8888/
                              the http/https URL where nodefoc is running
  --wallet-url TEXT=http://localhost:8888/
                              the http/https URL where kfocd is running
...
//copy pasta from kfocd's main.cpp
...
string default_wallet_url = "unix://" + (determine_home_directory() / "foc-wallet" / (string(key_store_executable_name) + ".sock")).string();
...
fc::variant push_transaction( signed_transaction& trx, packed_transaction::compression_type compression = packed_transaction::compression_type::none ) {
   ...
   if (trx.signatures.size() == 0) { // #5445 can't change txn content if already signed
      ...
      } FOC_RETHROW_EXCEPTIONS(invalid_ref_block_exception, "Invalid reference block num or id: ${block_num_or_id}", ("block_num_or_id", tx_ref_block_num_or_id));
...
fc::variant json_from_file_or_string(const string& file_or_str, fc::json::parse_type ptype = fc::json::legacy_parser)
{
   ...
   if ( !regex_search(file_or_str, r) && fc::is_regular_file(file_or_str) ) {
      ...
      } FOC_RETHROW_EXCEPTIONS(json_parse_exception, "Fail to parse JSON from file: ${file}", ("file", file_or_str));

   } else {
      try {
         return fc::json::from_string(file_or_str, ptype);
      } FOC_RETHROW_EXCEPTIONS(json_parse_exception, "Fail to parse JSON from string: ${string}", ("string", file_or_str));
   }
}
...
void send_actions(std::vector<chain::action>&& actions, packed_transaction::compression_type compression = packed_transaction::compression_type::none ) {
   ...
   if (tx_json_save_file.length()) {
      ...
      FOCC_ASSERT(!out.fail(), "ERROR: Failed to create file \"${p}\"", ("p", tx_json_save_file));
   }
...
authority parse_json_authority(const std::string& authorityJsonOrFile) {
   ...
   } FOC_RETHROW_EXCEPTIONS(authority_type_exception, "Invalid authority format '${data}'",
                            ("data", fc::json::to_string(authority_var, fc::time_point::maximum())))
}

authority parse_json_authority_or_key(const std::string& authorityJsonOrFile) {
   if (boost::istarts_with(authorityJsonOrFile, "FOC") || boost::istarts_with(authorityJsonOrFile, "PUB_R1")) {
      try {
         return authority(public_key_type(authorityJsonOrFile));
      } FOC_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", authorityJsonOrFile))
   } else {
      auto result = parse_json_authority(authorityJsonOrFile);
      FOC_ASSERT( eosio::chain::validate(result), authority_type_exception, "Authority failed validation! ensure that keys, accounts, and waits are sorted and that the threshold is valid and satisfiable!");
      return result;
   }
}
...
asset to_asset( account_name code, const string& s ) {
   ...
   if ( it == cache.end() ) {
      ...
      if (obj_it != obj.end()) {
         ...
         FOC_THROW(symbol_type_exception, "Symbol ${s} is not supported by token contract ${c}", ("s", sym_str)("c", code));
      }
   }
   ...
   if ( a.decimals() < expected_symbol.decimals() ) {
      ...
      FOC_THROW(symbol_type_exception, "Too many decimal digits in ${a}, only ${d} supported", ("a", a)("d", expected_symbol.decimals()));
...
struct set_account_permission_subcommand {
   ...
   set_account_permission_subcommand(CLI::App* accountCmd) {
      ...
      permissions->set_callback([this] {
         FOCC_ASSERT( !(add_code && remove_code), "ERROR: Either --add-code or --remove-code can be set" );
         FOCC_ASSERT( (add_code ^ remove_code) || !authority_json_or_file.empty(), "ERROR: authority should be specified unless add or remove code permission" );
...
void ensure_kfocd_running(CLI::App* app) {
    if (no_auto_kfocd)
        return;
    // get, version, net, convert do not require kfocd
    ...
    // This extra check is necessary when running clfoc like this: ./clfoc ...
    if (binPath.filename_is_dot())
        binPath.remove_filename();
    binPath.append(key_store_executable_name); // if clfoc and kfocd are in the same installation directory
    if (!boost::filesystem::exists(binPath)) {
        binPath.remove_filename().remove_filename().append("kfocd").append(key_store_executable_name);
    }
...
struct register_producer_subcommand {
   ...
   register_producer_subcommand(CLI::App* actionRoot) {
      ...
      register_producer->set_callback([this] {
         ...
         } FOC_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid producer public key: ${public_key}", ("public_key", producer_key_str))
   ...
   create_account_subcommand(CLI::App* actionRoot, bool s) : simple(s) {
      ...
      createAccount->set_callback([this] {
            ...
            if( owner_key_str.find('@') != string::npos ) {
               ...
               } FOC_RETHROW_EXCEPTIONS( explained_exception, "Invalid owner permission level: ${permission}", ("permission", owner_key_str) )
            } else {
               try {
                  owner = public_key_type(owner_key_str);
               } FOC_RETHROW_EXCEPTIONS( public_key_type_exception, "Invalid owner public key: ${public_key}", ("public_key", owner_key_str) );
            }
            ...
            if( active_key_str.empty() ) {
               ...
               } FOC_RETHROW_EXCEPTIONS( explained_exception, "Invalid active permission level: ${permission}", ("permission", active_key_str) )
            } else {
               try {
                  active = public_key_type(active_key_str);
               } FOC_RETHROW_EXCEPTIONS( public_key_type_exception, "Invalid active public key: ${public_key}", ("public_key", active_key_str) );
            }

            auto create = create_newaccount(name(creator), name(account_name), owner, active);
            if (!simple) {
               FOCC_ASSERT( buy_ram_eos.size() || buy_ram_bytes_in_kbytes || buy_ram_bytes, "ERROR: One of --buy-ram, --buy-ram-kbytes or --buy-ram-bytes should have non-zero value" );
               FOCC_ASSERT( !buy_ram_bytes_in_kbytes || !buy_ram_bytes, "ERROR: --buy-ram-kbytes and --buy-ram-bytes cannot be set at the same time" );
   ...
   approve_producer_subcommand(CLI::App* actionRoot) {
      ...
      approve_producer->set_callback([this] {
            auto result = call(get_table_func, fc::mutable_variant_object("json", true)
                               ...
                               // Less than ideal upper_bound usage preserved so clfoc can still work with old buggy nodefoc versions
                               // Change to voter.value when clfoc no longer needs to support nodefoc versions older than 1.5.0
                               ("limit", 1)
            );
            auto res = result.as<eosio::chain_apis::read_only::get_table_rows_result>();
            // Condition in if statement below can simply be res.rows.empty() when clfoc no longer needs to support nodefoc versions older than 1.5.0
            ...
            FOC_ASSERT( 1 == res.rows.size(), multiple_voter_info, "More than one voter_info for account" );
...
struct get_transaction_id_subcommand {
   ...
   get_transaction_id_subcommand(CLI::App* actionRoot) {
      ...
      get_transaction_id->set_callback([&] {
         ...
         } FOC_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trx_to_check))
      });
   }
};
...
struct delegate_bandwidth_subcommand {
   ...
   delegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      delegate_bandwidth->set_callback([this] {
         ...
         FOCC_ASSERT( !(buy_ram_amount.size()) || !buy_ram_bytes, "ERROR: --buyram and --buy-ram-bytes cannot be set at the same time" );
   ...
   bidname_info_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->set_callback([this] {
         auto rawResult = call(get_table_func, fc::mutable_variant_object("json", true)
                               ...
                               // Less than ideal upper_bound usage preserved so clfoc can still work with old buggy nodefoc versions
                               // Change to newname.value when clfoc no longer needs to support nodefoc versions older than 1.5.0
                               ("limit", 1));
         if ( print_json ) {
            std::cout << fc::json::to_pretty_string(rawResult) << std::endl;
            return;
...
struct buyram_subcommand {
   ...
   buyram_subcommand(CLI::App* actionRoot) {
      ...
      buyram->set_callback([this] {
         FOCC_ASSERT( !kbytes || !bytes, "ERROR: --kbytes and --bytes cannot be set at the same time" );
         if (kbytes || bytes) {
   ...
   if (!json_format) {
      ...
      if ( res.total_resources.is_object() ) {
         ...
         if( net_total.get_symbol() != unstaking.get_symbol() ) {
            // Core symbol of nodefoc responding to the request is different than core symbol built into clfoc
...
int main( int argc, char** argv ) {
   ...
   CLI::App app{"Command Line Interface to FOC Client"};
   ...
   app.add_flag( "--no-auto-" + string(key_store_executable_name), no_auto_kfocd, localized("don't automatically launch a ${k} if one is not currently running", ("k", key_store_executable_name)));
   app.set_callback([&app]{ ensure_kfocd_running(&app);});
   ...
   pack_transaction->set_callback([&] {
      ...
      if( pack_action_data_flag ) {
         ...
         } FOC_RETHROW_EXCEPTIONS( transaction_type_exception, "Invalid transaction format: '${data}'",
         ...
         } FOC_RETHROW_EXCEPTIONS( transaction_type_exception, "Fail to convert transaction, --pack-action-data likely needed" )
      }
   });
   ...
   unpack_transaction->set_callback([&] {
      ...
      } FOC_RETHROW_EXCEPTIONS( transaction_type_exception, "Invalid packed transaction format: '${data}'",
   ...
   pack_action_data->set_callback([&] {
      ...
      } FOC_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse unpacked action data JSON")
   ...
   unpack_action_data->set_callback([&] {
      FOC_ASSERT( packed_action_data_string.size() >= 2, transaction_type_exception, "No packed_action_data found" );
   ...
   getCode->set_callback([&] {
      ...
      try {
         ...
         //see if this is an old nodefoc that doesn't support get_raw_code_and_abi
   ...
   getAccounts->set_callback([&] {
      ...
      } FOC_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", public_key_str))
   ...
   getTransactions->set_callback([&] {
      ...
      if (skip_seq_str.empty()) {
         ...
         } FOC_RETHROW_EXCEPTIONS(chain_type_exception, "Invalid Skip Seq: ${skip_seq}", ("skip_seq", skip_seq_str))
         if (num_seq_str.empty()) {
            ...
            } FOC_RETHROW_EXCEPTIONS(chain_type_exception, "Invalid Num Seq: ${num_seq}", ("num_seq", num_seq_str))
...
//                                ->check(CLI::ExistingFile);
   ...
   auto set_code_callback = [&]() {
      ...
      if(!contract_clear){
        ...
        FOC_ASSERT( !wasm.empty(), wasm_file_not_found, "no wasm file found ${f}", ("f", wasmPath) );
   ...
   auto set_abi_callback = [&]() {
      ...
      if(!contract_clear){
        ...
        FOC_ASSERT( fc::exists( abiPath ), abi_file_not_found, "no abi file found ${f}", ("f", abiPath)  );
      ...
      if (!duplicate) {
         ...
         } FOC_RETHROW_EXCEPTIONS(abi_type_exception,  "Fail to parse ABI JSON")
         if ( shouldSend ) {
   ...
   contractSubcommand->set_callback([&] {
      if(!contract_clear) FOC_ASSERT( !contractPath.empty(), contract_exception, " contract-dir is null ", ("f", contractPath) );
      shouldSend = false;
      set_code_callback();
      set_abi_callback();
      if (actions.size()) {
   ...
   createWallet->set_callback([&wallet_name, &password_file, &print_console] {
      FOCC_ASSERT( !password_file.empty() ^ print_console, "ERROR: Either indicate a file using \"--file\" or pass \"--to-console\"" );
      FOCC_ASSERT( password_file.empty() || !std::ofstream(password_file.c_str()).fail(), "ERROR: Failed to create file in specified path" );
   ...
   importWallet->set_callback([&wallet_name, &wallet_key_str] {
      ...
      try {
         ...
         FOC_THROW(private_key_type_exception, "Invalid private key")
      }
   ...
   removeKeyWallet->set_callback([&wallet_name, &wallet_pw, &wallet_rm_key_str] {
      ...
      try {
         ...
         FOC_THROW(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", wallet_rm_key_str))
      }
   ...
   stopKeosd->set_callback([] {
      const auto& v = call(wallet_url, kfocd_stop);
      if ( !v.is_object() || v.get_object().size() != 0 ) { //on success kfocd responds with empty object
   ...
   sign->set_callback([&] {
      ...
      FOCC_ASSERT( str_private_key.empty() || str_public_key.empty(), "ERROR: Either -k/--private-key or --public-key or none of them can be set" );
      ...
      if( str_public_key.size() > 0 ) {
         ...
         } FOC_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", str_public_key))
         ...
         } FOC_RETHROW_EXCEPTIONS(private_key_type_exception, "Invalid private key")
   ...
   propose_action->set_callback([&] {
      ...
      } FOC_RETHROW_EXCEPTIONS(transaction_type_exception, "Wrong requested permissions format: '${data}'", ("data",requested_perm_var));

      vector<permission_level> trxperm;
      try {
         trxperm = transaction_perm_var.as<vector<permission_level>>();
      } FOC_RETHROW_EXCEPTIONS(transaction_type_exception, "Wrong transaction permissions format: '${data}'", ("data",transaction_perm_var));
      ...
      if (accountPermissions.empty()) {
         if (!proposer.empty()) {
            ...
            FOC_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <proposer> or -p)");
         }
      }
      if (proposer.empty()) {
   ...
   review->set_callback([&] {
      const auto result1 = call(get_table_func, fc::mutable_variant_object("json", true)
                                 ...
                                 // Less than ideal upper_bound usage preserved so clfoc can still work with old buggy nodefoc versions
                                 // Change to name(proposal_name).value when clfoc no longer needs to support nodefoc versions older than 1.5.0
                                 ("limit", 1)
                           );
      //std::cout << fc::json::to_pretty_string(result) << std::endl;

      const auto& rows1 = result1.get_object()["rows"].get_array();
      // Condition in if statement below can simply be rows.empty() when clfoc no longer needs to support nodefoc versions older than 1.5.0
   ...
   cancel->set_callback([&]() {
      ...
      if (accountPermissions.empty()) {
         if (!canceler.empty()) {
            ...
            FOC_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <canceler> or -p)");
         }
      }
      if (canceler.empty()) {
   ...
   exec->set_callback([&] {
      ...
      if (accountPermissions.empty()) {
         if (!executer.empty()) {
            ...
            FOC_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <executer> or -p)");
         }
      }
      if (executer.empty()) {
