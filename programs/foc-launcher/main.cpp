...
struct launcher_def {
   ...
   std::map<uint,string> specific_nodefoc_args;
   std::map<uint,string> specific_nodefoc_installation_paths;
   testnet_def network;
   string gelf_endpoint;
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  retrieve_paired_array_parameters(vmap, "specific-num", "specific-" + string(node_executable_name), specific_nodefoc_args);
  retrieve_paired_array_parameters(vmap, "spcfc-inst-num", "spcfc-inst-" + string(node_executable_name), specific_nodefoc_installation_paths);
  ...
  if (vmap.count("specific-num")) {
    ...
    for(uint i = 0; i < specific_nums.size(); ++i)
    {
      ...
      specific_nodefoc_args[num] = specific_args[i];
    }
  }

  char* erd_env_var = getenv ("FOC_HOME");
  ...
  stage = bfs::path(erd);
  if (!bfs::exists(stage)) {
    cerr << "\"" << erd << "\" is not a valid path. Please ensure environment variable FOC_HOME is set to the build path." << endl;
    exit (-1);
  }
...
launcher_def::launch (eosd_def &instance, string &gts) {
  ...
  string install_path;
  if (instance.name != "bios" && !specific_nodefoc_installation_paths.empty()) {
     const auto node_num = boost::lexical_cast<uint16_t,string>(instance.get_node_num());
     if (specific_nodefoc_installation_paths.count(node_num)) {
        install_path = specific_nodefoc_installation_paths[node_num] + "/";
     }
  }
  string eosdcmd = install_path + "programs/nodefoc/" + string(node_executable_name) + " ";
  ...
  if (instance.name != "bios" && !specific_nodefoc_args.empty()) {
     const auto node_num = boost::lexical_cast<uint16_t,string>(instance.get_node_num());
     if (specific_nodefoc_args.count(node_num)) {
        eosdcmd += specific_nodefoc_args[node_num] + " ";
     }
  }
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string cmd = "./scripts/foc-tn_bounce.sh " + eosd_extra_args;
      if (node_num != "bios" && !specific_nodefoc_args.empty()) {
         const auto node_num_i = boost::lexical_cast<uint16_t,string>(node_num);
         if (specific_nodefoc_args.count(node_num_i)) {
            cmd += " " + specific_nodefoc_args[node_num_i];
         }
      }

      do_command(host, node.name, { { "FOC_HOME", host.eosio_home }, { "FOC_NODE", node_num } }, cmd);
   }
}
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string cmd = "./scripts/foc-tn_down.sh ";
      do_command(host, node.name,
                 { { "FOC_HOME", host.eosio_home }, { "FOC_NODE", node_num }, { "FOC_TN_RESTART_CONFIG_DIR", node.config_dir_name } },
                 cmd);
   }
}
...
launcher_def::roll (const string& host_names) {
   ...
   for (string host_name: hosts) {
      ...
      string cmd = "./scripts/foc-tn_roll.sh ";
      do_command(*host, host_name, { { "FOC_HOME", host->eosio_home } }, cmd);
   }
}
...
int main (int argc, char *argv[]) {
  ...
  cli.add_options()
    ...
    ("down", bpo::value<string>(&down_nodes),"comma-separated list of node numbers that will be taken down using the foc-tn_down.sh script")
    ("bounce", bpo::value<string>(&bounce_nodes),"comma-separated list of node numbers that will be restarted using the foc-tn_bounce.sh script")
    ("roll", bpo::value<string>(&roll_nodes),"comma-separated list of host names where the nodes should be rolled to a new version using the foc-tn_roll.sh script")
